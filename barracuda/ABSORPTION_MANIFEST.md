...
**Date:** April 27, 2026 (V150 Phase 46 composition explorer; detailed tables from V137)
...
**Status:** 150+ primitives consumed, 44 GPU modules (all lean), 0 local WGSL, 0 local ODE/regression math, **1,594** lib tests + **18** IPC roundtrip, 383 indexed (380 completed + 3 proposed), 5,900+ checks, **364** binaries (342 barracuda + 22 forge), 242 named tolerances (8 with run commands), `forbid(unsafe_code)` at workspace level + per-crate, clippy pedantic+nursery CLEAN, **zero `#[allow()]`**. 91.20% coverage (gated at 90%). **42** niche capabilities across **21** domains (8 families). **48** consumed capabilities (33 v0.9.17 canonical + 15 legacy). **136/136** proto-nucleate alignment (Exp400 D01–D07 guard constant). **7** deploy graphs (canonical schema). **15** composition gaps open, **7** resolved (`docs/PRIMAL_GAPS.md`, PG-01–PG-22). **guideStone Level 4:** 38/38 pass, 4 skip — `is_skip_error` from v0.9.17, BLAKE3 Property 3. **V149:** v0.9.17 alignment, 6 methods resolved, `is_skip_error` adopted. **V148:** Level 4 initial (31/31). **V141:** Capability alignment. **See also:** `wateringHole/handoffs/WETSPRING_V149_V0917_ALIGNMENT_HANDOFF_APR20_2026.md`.
