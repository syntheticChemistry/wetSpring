...
**Date:** May 12, 2026 (V165b; detailed tables from V137)
...
**Status:** 150+ primitives consumed, 44 GPU modules (all lean), 0 local WGSL, 0 local ODE/regression math, **1,962** lib tests + **97** integration + **18** IPC roundtrip, 384 indexed (381 completed + 3 proposed), 5,900+ checks, **367** binaries (345 barracuda + 22 forge), 242 named tolerances (8 with run commands), `forbid(unsafe_code)` at workspace level + per-crate, clippy pedantic+nursery CLEAN, **zero `#[allow()]`**, one justified `dyn` exception (`gillespie.rs` SSA). 91.20% coverage (gated at 90%). **42** niche capabilities across **21** domains (8 families). **48** consumed capabilities (33 v0.9.17 canonical + 15 legacy, machine-readable in `niche.rs`). **136/136** proto-nucleate alignment (Exp400 D01–D07 guard constant). **7** deploy graphs (canonical schema). **4** composition gaps open (all external), **18** resolved/closed (`docs/PRIMAL_GAPS.md`, PG-01–PG-22). Tier 4 IPC-first defaults. **guideStone Level 4:** 38/38 pass, 4 skip — `is_skip_error` from v0.9.17, BLAKE3 Property 3. **V165b:** Tier 2 IPC wired. **V165:** LTEE B7 Tier 2 (27/27 PASS). **V164b:** Foundation 10/10 threads active. **V164:** Thread 4 expression, `--format json`, projectNUCLEUS workload. **V162:** PG-12 resolved (legacy surface separated), foundation seeded. **V160:** PG-09 resolved. **V159:** barraCuda IPC routing.
