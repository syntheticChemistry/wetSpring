...
**Date:** May 29, 2026 (V190; detailed tables from V137)
...
**Status:** 150+ primitives consumed, 44 GPU modules (all lean), 0 local WGSL, 0 local ODE/regression math, **2,085** tests, 386 experiments (385 done + 1 in progress), 5,967+ checks, **1** UniBin (345 scenarios), 242 named tolerances (8 with run commands), `forbid(unsafe_code)` at workspace level + per-crate, clippy pedantic+nursery 4 warnings (casts), one justified `dyn` exception (`gillespie.rs` SSA). 91.20% coverage (gated at 90%). **50** niche capabilities across **22** domains (8 families). **59** consumed capabilities (33 canonical + 15 legacy + 7 bonding/lifecycle + 4 Wave 17/20, machine-readable in `niche.rs`). **136/136** proto-nucleate alignment (Exp400 D01–D07 guard constant). **7** deploy graphs (canonical schema). **0** composition gaps open, **22** resolved/closed (`docs/PRIMAL_GAPS.md`, PG-01–PG-22; PG-02/PG-04 VERIFIED). Tier 4 IPC-first defaults. **guideStone Level 5:** 38/38 pass, 4 environmental skip — primal proof. **V190:** Wave 60 eukaryotic, 13/13 NUCLEUS processes (11/13 health, 2 BTSP-gated), biomeOS 1725 capabilities, Forgejo remotes. **V189:** Wave 60 first deployment, 13/13 processes. **V188:** Wave 55 southGate redeploy. **V187:** Wave 50 covalent HPC. **V186:** plasmidBin-only binary discovery. **V185:** Wave 48 behavioral convergence.
