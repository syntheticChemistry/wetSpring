...
**Date:** May 25, 2026 (V187; detailed tables from V137)
...
**Status:** 150+ primitives consumed, 44 GPU modules (all lean), 0 local WGSL, 0 local ODE/regression math, **1,962** lib tests + **97** integration + **18** IPC roundtrip, 386 experiments (385 done + 1 in progress), 5,967+ checks, **1** UniBin (345 scenarios), 242 named tolerances (8 with run commands), `forbid(unsafe_code)` at workspace level + per-crate, clippy pedantic+nursery CLEAN, **zero `#[allow()]`**, one justified `dyn` exception (`gillespie.rs` SSA). 91.20% coverage (gated at 90%). **50** niche capabilities across **22** domains (8 families). **59** consumed capabilities (33 canonical + 15 legacy + 7 bonding/lifecycle + 4 Wave 17/20, machine-readable in `niche.rs`). **136/136** proto-nucleate alignment (Exp400 D01–D07 guard constant). **7** deploy graphs (canonical schema). **2** composition gaps open (deployment-only), **20** resolved/closed (`docs/PRIMAL_GAPS.md`, PG-01–PG-22). Tier 4 IPC-first defaults. **guideStone Level 5:** 38/38 pass, 4 environmental skip — primal proof. **V187:** Wave 50 covalent HPC, post-primordial, NUCLEUS 7/13 health-responding. **V186:** plasmidBin-only binary discovery, shared `primal_binary` module. **V185:** Wave 48 behavioral convergence, cell deployment, Songbird TCP 0.0.0.0:7700.
