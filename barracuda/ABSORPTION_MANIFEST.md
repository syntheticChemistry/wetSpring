...
**Date:** May 17, 2026 (V174; detailed tables from V137)
...
**Status:** 150+ primitives consumed, 44 GPU modules (all lean), 0 local WGSL, 0 local ODE/regression math, **1,962** lib tests + **97** integration + **18** IPC roundtrip, 384/384 completed, 5,957+ checks, **370** binaries (348 barracuda + 22 forge), 242 named tolerances (8 with run commands), `forbid(unsafe_code)` at workspace level + per-crate, clippy pedantic+nursery CLEAN, **zero `#[allow()]`**, one justified `dyn` exception (`gillespie.rs` SSA). 91.20% coverage (gated at 90%). **42** niche capabilities across **21** domains (8 families). **51** consumed capabilities (33 v0.9.17 canonical + 15 legacy + 3 Wave 17/20, machine-readable in `niche.rs`). **136/136** proto-nucleate alignment (Exp400 D01–D07 guard constant). **7** deploy graphs (canonical schema). **2** composition gaps open (deployment-only), **20** resolved/closed (`docs/PRIMAL_GAPS.md`, PG-01–PG-22). Tier 4 IPC-first defaults. **guideStone Level 5:** 38/38 pass, 4 environmental skip — primal proof, `is_skip_error` from v0.9.17, BLAKE3 Property 3. **V174:** Exp377–379 hormesis chain (57/57 PASS), deep debt resolved (UniBin `serve` live, display-name hygiene, `dense_to_csr` extracted). **V171:** Live composition health: runtime probing replaces deferred checks, Wave 20 schema parity. **V169:** Wave 17 signal adoption. **V168:** barraCuda v0.4.0 absorbed, coralReef v0.1.0 niche, live NUCLEUS 30/31 pass.
