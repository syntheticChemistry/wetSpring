...
fn squirrel_query(socket_path: &Path, query: &str, context: &Value) -> Result<Value, String> {
    let request = json!({
        "jsonrpc": "2.0",
        "method": "ai.query",
        "params": {
    ...
    let parsed: Value = serde_json::from_str(line.trim()).map_err(|e| format!("parse: {e}"))?;
...
pub fn handle_ai_ecology_interpret(params: &Value) -> Result<Value, RpcError> {
    ...
    match squirrel_query(&socket, query, &context) {
        ...
        Err(e) => {
            let (status, message) = if e.contains("timeout") || e.contains("timed out") {
