...
use wetspring_barracuda::bio::ncbi_data::{load_vibrio_assemblies, VibrioAssembly};
...
fn derive_params(assembly: &VibrioAssembly) -> QsBiofilmParams {
    ...
    p.mu_max = (1.2 - (assembly.genome_size_bp as f64 - 3_500_000.0) / 5_000_000.0 * 1.0).clamp(0.2, 1.2);
    p.k_ai_prod = (assembly.gene_count as f64 / 1000.0).clamp(0.5, 15.0);
    let density = assembly.gene_count as f64 / (assembly.genome_size_bp as f64 / 1e6);
    p.k_hapr_ai = (density / 500.0).clamp(0.1, 2.0);
    p
}
...
fn main() {
    ...
    let mu_min = derived_params.iter().map(|p| p.mu_max).fold(f64::INFINITY, f64::min);
    let mu_max = derived_params.iter().map(|p| p.mu_max).fold(f64::NEG_INFINITY, f64::max);
    let kai_min = derived_params.iter().map(|p| p.k_ai_prod).fold(f64::INFINITY, f64::min);
    let kai_max = derived_params.iter().map(|p| p.k_ai_prod).fold(f64::NEG_INFINITY, f64::max);
    println!("  mu_max range: [{:.3}, {:.3}]", mu_min, mu_max);
    println!("  k_ai_prod range: [{:.3}, {:.3}]", kai_min, kai_max);

    let clinical_count = assemblies.iter().filter(|a| is_clinical(&a.isolation_source)).count();
    let environmental_count = assemblies.len() - clinical_count;
    println!("  clinical: {clinical_count}, environmental: {environmental_count}");
    v.check_count("mu_max in [0.2, 1.2]", usize::from(mu_min >= 0.2 && mu_max <= 1.2), 1);
    v.check_count("clinical or environmental present", usize::from(clinical_count > 0 || environmental_count > 0), 1);
    ...
    let cpu_results: Vec<[f64; 5]> = derived_params
        ...
        .map(|p| {
            ...
            [r.y_final[0], r.y_final[1], r.y_final[2], r.y_final[3], r.y_final[4]]
        })
        .collect();
    ...
    #[cfg(feature = "gpu")]
    {
        ...
        let all_params: Vec<f64> = derived_params.iter().flat_map(|p| params_to_flat_17(p)).collect();
        ...
        println!("  biofilm: {}, planktonic: {}, extinction: {}, intermediate: {}",
        ...
        v.check_count("landscape has biofilm", usize::from(gpu_classes.get("biofilm").copied().unwrap_or(0) > 0), 1);
        ...
        println!("  Landscape diversity: {} distinct outcomes{}", gpu_classes.len(),
            if !diverse { " (all biofilm — real Vibrio cluster in biofilm space)" } else { "" });
    ...
    let clinical_biofilm: usize = assemblies
        ...
        .count();
    let clinical_total: usize = assemblies.iter().filter(|a| is_clinical(&a.isolation_source)).count();
    ...
    let clinical_frac = if clinical_total > 0 { clinical_biofilm as f64 / clinical_total as f64 } else { 0.0 };
    let env_frac = if env_total > 0 { env_biofilm as f64 / env_total as f64 } else { 0.0 };
    println!("  clinical biofilm fraction: {:.3} ({clinical_biofilm}/{clinical_total})", clinical_frac);
    println!("  environmental biofilm fraction: {:.3} ({env_biofilm}/{env_total})", env_frac);
    ...
    println!("  Exp121 real-derived: {n} assemblies, biofilm={biofilm_count}, planktonic={planktonic_count}");
