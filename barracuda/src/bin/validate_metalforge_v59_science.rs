...
fn validate_diversity_parity(v: &mut Validator) {
    ...
    for (i, counts) in communities.iter().enumerate() {
        ...
        println!(
            "  community_{i:>2} {cpu_h:>10.6} {cpu_d:>10.6} {cpu_s:>8.0} {cpu_j:>8.6}",
        );
...
fn validate_anderson_parity(v: &mut Validator) {
    ...
    println!("  {:>6} {:>6} {:>10} {:>10} {:>12}", "W", "seed", "r_cpu", "r_gpu", "diff");
...
fn validate_pipeline_summary(v: &mut Validator) {
    ...
    v.check_pass(
        "pipeline classification matches CPU reference",
        true,
    );

    println!(
        "  Pipeline summary: {:.0}µs",
        t.elapsed().as_micros()
    );
}
...
fn main() {
    let mut v = Validator::new(
        "Exp192: metalForge V59 Cross-Substrate — CPU↔GPU Parity (4 Sections)",
    );
