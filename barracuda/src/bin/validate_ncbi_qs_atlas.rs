...
use barracuda::spectral::{
    anderson_hamiltonian, find_all_eigenvalues, level_spacing_ratio, GOE_R, POISSON_R,
};
...
fn main() {
    ...
    println!("  BioProjects: {}, biome params: {} ({load_ms:.1} ms)", projects.len(), params.len());
    v.check_pass(">= 20 biomes profiled", params.len() >= 20);

    v.section("── S2: Diversity computation ──");
    let mut atlas: Vec<(String, f64, f64, f64, Option<f64>, Option<&'static str>)> = Vec::new();
    ...
    v.check_count("Pielou J computed for all biomes", j_vals.len(), params.len());

    v.section("── S3: Disorder mapping ──");
    for (name, _j, w, _, _, _) in &atlas {
        v.check_pass(&format!("{name} W in [0.5, 15.0]"), *w >= 0.5 && *w <= 15.0);
    }
    let mut sorted_by_j: Vec<(f64, f64)> = atlas.iter().map(|(_, j, w, _, _, _)| (*j, *w)).collect();
    ...
    #[cfg(feature = "gpu")]
    {
        ...
        for (name, _, w, _, r_opt, regime_opt) in &mut atlas {
            ...
            *r_opt = Some(r);
            *regime_opt = Some(if r > midpoint { "QS-active" } else { "QS-suppressed" });
            v.check_pass(
        ...
        println!("  Known high-QS: {high_correct}/{high_total} correct ({:.1}%)", high_acc * 100.0);
        println!("  Known low-QS: {low_correct}/{low_total} correct ({:.1}%)", low_acc * 100.0);
        v.check_pass(
        ...
        let active: Vec<_> = atlas.iter().filter(|(_, _, _, _, _, r)| *r == Some("QS-active")).collect();
        ...
        for (name, j, w, _, r_opt, _) in &active {
            println!("    {name}: J={j:.3} W={w:.2} ⟨r⟩={:.4}", r_opt.unwrap_or(0.0));
        }
        ...
        v.check_pass("mean W(QS-active) < mean W(QS-suppressed)", mean_w_active < mean_w_suppressed);
    }
