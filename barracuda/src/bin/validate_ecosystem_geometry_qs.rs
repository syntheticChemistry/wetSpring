...
fn main() {
    ...
    #[cfg(feature = "gpu")]
    {
        ...
        let zones = [
            // ── Cave ecosystems ──
            EcoZone {
                ecosystem: "cave", zone: "wall_biofilm",
                n_species: 80, j_target: 0.45,
                lx: 16, ly: 16, lz: 0,
                rationale: "thin mineral crust, low-moderate diversity",
            },
            EcoZone {
                ecosystem: "cave", zone: "passage_sediment",
                n_species: 200, j_target: 0.65,
                lx: 7, ly: 7, lz: 7,
                rationale: "3D mud/clay pore network",
            },
            EcoZone {
                ecosystem: "cave", zone: "stalactite_film",
                n_species: 30, j_target: 0.25,
                lx: 48, ly: 3, lz: 3,
                rationale: "cylindrical drip-fed biofilm",
            },
            EcoZone {
                ecosystem: "cave", zone: "subterranean_river",
                n_species: 120, j_target: 0.55,
                lx: 24, ly: 4, lz: 4,
                rationale: "tubular water channel with sediment",
            },
            // ── Hot spring ecosystems ──
            EcoZone {
                ecosystem: "hot_spring", zone: "surface_mat",
                n_species: 40, j_target: 0.30,
                lx: 16, ly: 16, lz: 2,
                rationale: "cyanobacterial photosynthetic surface, 2-3 cell layers",
            },
            EcoZone {
                ecosystem: "hot_spring", zone: "deep_mat",
                n_species: 150, j_target: 0.60,
                lx: 10, ly: 10, lz: 4,
                rationale: "anaerobic heterotroph zone, 4+ cell layers",
            },
            EcoZone {
                ecosystem: "hot_spring", zone: "silica_sinter",
                n_species: 25, j_target: 0.20,
                lx: 14, ly: 14, lz: 0,
                rationale: "mineral encrustation, very low diversity",
            },
            EcoZone {
                ecosystem: "hot_spring", zone: "pool_sediment",
                n_species: 300, j_target: 0.70,
                lx: 7, ly: 7, lz: 8,
                rationale: "3D reducing sediment, high diversity",
            },
            // ── Rhizosphere ecosystems ──
            EcoZone {
                ecosystem: "rhizosphere", zone: "root_surface",
                n_species: 50, j_target: 0.35,
                lx: 14, ly: 14, lz: 2,
                rationale: "root exudate-selected specialists, thin biofilm",
            },
            EcoZone {
                ecosystem: "rhizosphere", zone: "inner_rhizosphere",
                n_species: 200, j_target: 0.55,
                lx: 8, ly: 8, lz: 6,
                rationale: "1-2mm from root, moderate diversity, 3D structure",
            },
            EcoZone {
                ecosystem: "rhizosphere", zone: "bulk_soil",
                n_species: 1000, j_target: 0.90,
                lx: 7, ly: 7, lz: 8,
                rationale: "3D pore network, extreme diversity",
            },
            EcoZone {
                ecosystem: "rhizosphere", zone: "mycorrhizal_hyphae",
                n_species: 60, j_target: 0.40,
                lx: 30, ly: 3, lz: 4,
        ...
        for zone in zones.iter().filter(|z| z.ecosystem == "cave") {
            ...
            let shape = if zone.lz == 0 { format!("{}×{}", zone.lx, zone.ly) }
                else { format!("{}×{}×{}", zone.lx, zone.ly, zone.lz) };
            println!("  {}: J={j:.3} W={w:.2} shape={shape} N={n_sites} ⟨r⟩={r:.4} → {regime}  [{}]",
                zone.zone, zone.rationale);
            cave_results.push((zone.zone, r, regime));
            v.check_pass(&format!("cave {} computed", zone.zone), true);
        }
        let cave_wall_regime = cave_results.iter()
            .find(|(n, _, _)| *n == "wall_biofilm")
            .map(|(_, r, _)| *r).unwrap_or(0.0);
        let cave_sediment_r = cave_results.iter()
            .find(|(n, _, _)| *n == "passage_sediment")
            .map(|(_, r, _)| *r).unwrap_or(0.0);
        v.check_pass("cave sediment (3D) ⟨r⟩ > cave wall (2D) ⟨r⟩", cave_sediment_r > cave_wall_regime || (cave_sediment_r - cave_wall_regime).abs() < 0.05);
        ...
        let pool_r = hs_results.iter().find(|(n, _, _)| *n == "pool_sediment").map(|(_, r, _)| *r).unwrap_or(0.0);
        let sinter_r = hs_results.iter().find(|(n, _, _)| *n == "silica_sinter").map(|(_, r, _)| *r).unwrap_or(0.0);
        // Pool sediment (3D, high diversity) vs silica sinter (2D, low diversity):
        // 3D geometry overcomes higher disorder
        v.check_pass("pool sediment (3D) ⟨r⟩ > silica sinter (2D) despite higher diversity", pool_r > sinter_r);
        ...
        let root_r = rz_results.iter().find(|(n, _, _)| *n == "root_surface").map(|(_, r, _)| *r).unwrap_or(0.0);
        let bulk_r = rz_results.iter().find(|(n, _, _)| *n == "bulk_soil").map(|(_, r, _)| *r).unwrap_or(0.0);
        v.check_pass("root surface and bulk soil both computed", root_r > 0.0 && bulk_r > 0.0);

        v.section("── S4: Cross-ecosystem comparison ──");
        let all: Vec<_> = cave_results.iter()
            .chain(hs_results.iter())
            .chain(rz_results.iter())
            .collect();
        let active_count = all.iter().filter(|(_, _, reg)| *reg == "QS-ACTIVE").count();
        let suppressed_count = all.iter().filter(|(_, _, reg)| *reg == "suppressed").count();
        println!("  Total zones: {}, QS-ACTIVE: {}, suppressed: {}", all.len(), active_count, suppressed_count);
        ...
        println!("    - Thin surface mats show QS only when diversity is low (cyanobacteria-dominated)");
