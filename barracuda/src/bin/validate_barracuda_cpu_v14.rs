...
use wetspring_barracuda::bio::{
    bistable, capacitor, cooperation, derep, diversity, dnds, felsenstein, gillespie,
    hmm, kmd, kmer, merge_pairs, multi_signal, neighbor_joining, pangenome, pcoa,
    phage_defense, quality, qs_biofilm, reconciliation, robinson_foulds, signal, snp,
...
fn main() {
    ...
    v.check_pass("DF64 π roundtrip < 1e-14", pi_err < 1e-14);
    ...
    v.check("Shannon(uniform 4) = ln(4)", diversity::shannon(&[1.0; 4]), 4.0_f64.ln(), tolerances::PYTHON_PARITY);
    v.check("Simpson(uniform 4) = 0.75", diversity::simpson(&[1.0; 4]), 0.75, tolerances::PYTHON_PARITY);
    v.check("BC self == 0", diversity::bray_curtis(&[10.0], &[10.0]), 0.0, tolerances::EXACT);

    v.section("D02: Quality + Dereplication");
    total_domains += 2;
    let params = quality::QualityParams {
        window_size: 4, window_min_quality: 15, leading_min_quality: 3,
        trailing_min_quality: 3, min_length: 36, phred_offset: 33,
    };
    v.check_pass("QualityParams constructible", params.window_size == 4);
    v.check_pass("DerepSort accessible", matches!(derep::DerepSort::Abundance, derep::DerepSort::Abundance));

    v.section("D03: Merge Pairs");
    total_domains += 1;
    v.check_pass("revcomp(ATGC) == GCAT", merge_pairs::reverse_complement(b"ATGC") == b"GCAT");
    v.check_pass("revcomp involution", merge_pairs::reverse_complement(b"GCAT") == b"ATGC");
    ...
    v.check_pass("RF > 0 for different topologies", robinson_foulds::rf_distance(&t1, &t2) > 0);
    ...
    v.check_pass("JC distance > 0", neighbor_joining::jukes_cantor_distance(b"ATGCATGC", b"ATGGATGC") > 0.0);
    ...
    let costs = reconciliation::DtlCosts { duplication: 1, transfer: 2, loss: 1 };
    let tip_map = vec![("g1".to_string(), "h1".to_string()), ("g2".to_string(), "h2".to_string())];
    ...
    v.check("row sum = 1", p_same + 3.0 * p_diff, 1.0, tolerances::ANALYTICAL_F64);
    ...
    v.check_pass("omega computed", dnds_result.omega.is_none_or(f64::is_finite));
    ...
    total_domains += 1;
    let clusters = vec![
        pangenome::GeneCluster { id: "core1".into(), presence: vec![true, true, true] },
        pangenome::GeneCluster { id: "acc1".into(), presence: vec![true, true, false] },
        pangenome::GeneCluster { id: "unique1".into(), presence: vec![false, false, true] },
    ];
    ...
    v.section("D12: K-mer");
    total_domains += 1;
    v.check_pass("k-mer table populated", kmer::count_kmers(b"ATGCATGC", 3).total_valid_kmers > 0);
    ...
    v.check_pass("QS ODE: N > 0", *qs_r.states().last().unwrap().first().unwrap() > 0.0);

    let coop_r = cooperation::scenario_equal_start(&cooperation::CooperationParams::default(), 0.01);
    v.check_pass("Cooperation: freq finite", cooperation::cooperator_frequency(&coop_r).last().unwrap().is_finite());

    let bi_r = bistable::run_bistable(&[0.01, 0.0, 0.0, 2.0, 0.5], 0.01, 100.0, &bistable::BistableParams::default());
    v.check_pass("Bistable: trajectory", bi_r.t.len() > 1);

    let ph_r = phage_defense::scenario_no_phage(&phage_defense::PhageDefenseParams::default(), 0.01);
    ...
    v.check_pass("No-till QS > tilled QS", norm_cdf((w_c - notill_w) / 3.0) > norm_cdf((w_c - tilled_w) / 3.0));
    v.check_pass("Recovery W(31yr) < W(0)", 18.0 * (-31.0 / 10.0_f64).exp() < 18.0);
    v.check_pass("Pore gradient: big > small QS", norm_cdf((w_c - 25.0 * (1.0 - 0.85)) / 3.0) > norm_cdf((w_c - 25.0 * (1.0 - 0.2)) / 3.0));
    v.check_pass("Aggregate interior: large > small", (98.0_f64).powi(3) / (100.0_f64).powi(3) > (8.0_f64).powi(3) / (10.0_f64).powi(3));

    v.section("D24: Spectral Match");
    total_domains += 1;
    let mz = [100.0, 200.0, 300.0];
    let int = [1000.0, 500.0, 200.0];
    v.check("cosine self == 1", spectral_match::cosine_similarity(&mz, &int, &mz, &int, 0.5).score, 1.0, tolerances::ANALYTICAL_F64);

    v.section("D25: KMD");
    total_domains += 1;
    v.check_pass("KMD results", kmd::kendrick_mass_defect(&[200.0, 214.015_650_64], kmd::units::CH2_EXACT, kmd::units::CH2_NOMINAL).len() == 2);

    v.section("D26: Signal Processing");
    total_domains += 1;
    v.check_pass("peaks detected", !signal::find_peaks(&[0.0, 1.0, 3.0, 1.0, 0.0, 2.0, 5.0, 2.0, 0.0], &signal::PeakParams::default()).is_empty());
    ...
    v.check_pass("UniFrac in [0,1]", (0.0..=1.0).contains(&unifrac::distance::unweighted_unifrac(&tree, &s1, &s2)));

    v.section("D28: PCoA");
    total_domains += 1;
    let dm = [0.5, 0.8, 0.6];
    v.check_pass("PCoA produces coords", pcoa::pcoa(&dm, 3, 2).unwrap().n_samples == 3);
    ...
    let row_sums: Vec<f64> = (0..n_g).map(|i| (0..n_g).map(|j| lap[i * n_g + j]).sum()).collect();
    v.check_pass("Laplacian row sums ≈ 0", row_sums.iter().all(|s| s.abs() < 1e-10));
    v.check_pass("Laplacian diagonal ≥ 0", (0..n_g).all(|i| lap[i * n_g + i] >= 0.0));
    ...
    let nmf_cfg = barracuda::linalg::nmf::NmfConfig {
        rank: 3, max_iter: 100, tol: 1e-4,
        objective: barracuda::linalg::nmf::NmfObjective::KlDivergence, seed: 42,
    };
    ...
    v.check_pass("ridge weights finite", ridge.weights.iter().all(|w| w.is_finite()));
    ...
    v.check_pass("Anderson eigenvalues finite", eigs_a.iter().all(|e: &f64| e.is_finite()));
    ...
    v.check("Pearson(linear) = 1.0", pearson, 1.0, tolerances::ANALYTICAL_F64);

    let trapz = barracuda::numerical::trapz(
        &(0..1001).map(|i| { let x = f64::from(i) / 1000.0; x * x }).collect::<Vec<_>>(),
        &(0..1001).map(|i| f64::from(i) / 1000.0).collect::<Vec<_>>(),
    ).expect("trapz");
    v.check("trapz(x²) ≈ 1/3", trapz, 1.0 / 3.0, tolerances::TRAPZ_COARSE);

    // ═══ Summary ═════════════════════════════════════════════════════
    v.section(&format!("Summary: {total_domains} domains validated"));
    println!("  Core 47 domains (v13) + 3 cross-spring + 3 ToadStool + DF64 = {total_domains} total");
