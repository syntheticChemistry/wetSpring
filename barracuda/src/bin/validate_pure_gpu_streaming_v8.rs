...
fn main() {
    ...
    let communities: Vec<Vec<f64>> = (0..n_communities).map(|seed| {
        (0..n_taxa).map(|j| ((seed * 17 + j * 11 + 3) % 80 + 1) as f64).collect()
    }).collect();
    ...
    let gpu_shannons: Vec<f64> = communities.iter()
        ...
        .collect();
    let cpu_shannons: Vec<f64> = communities.iter()
    ...
    for i in 0..n_communities {
        v.check(
            ...
            gpu_shannons[i], cpu_shannons[i], tolerances::GPU_VS_CPU_F64,
        );
    }
    println!("  Stage 1: {} communities, {s1_ms:.2} ms", n_communities);
    ...
    v.check_pass("S2: CI contains estimate", ci.lower <= ci.estimate && ci.estimate <= ci.upper);
    println!("  Stage 2: CI = {:.4} [{:.4}, {:.4}], {s2_ms:.2} ms", ci.estimate, ci.lower, ci.upper);
    ...
    v.check("S3: Jackknife ≈ Bootstrap estimate", jk.estimate, ci.estimate, 0.01);
    v.check_pass("S3: Jackknife SE > 0", jk.std_error > 0.0);
    v.check_pass("S3: Bootstrap SE ≈ Jackknife SE (3×)",
        ci.std_error / jk.std_error < 3.0 && jk.std_error / ci.std_error < 3.0);
    println!("  Stage 3: JK = {:.4} ± {:.6}, {s3_ms:.2} ms", jk.estimate, jk.std_error);
    ...
    let cumulative_diversity: Vec<f64> = (1..=n_communities).map(|n| {
        let subset_mean = gpu_shannons[..n].iter().sum::<f64>() / n as f64;
        subset_mean
    }).collect();
    ...
    if let Some(b) = best {
        ...
        println!("  Stage 4: best model = {} (R²={:.4}), {s4_ms:.2} ms", b.model, b.r_squared);
        for f in &all_fits {
    ...
    let bc_condensed = diversity::bray_curtis_condensed(&communities[..10].to_vec());
    ...
    let sites: Vec<kriging::SpatialSample> = communities.iter().enumerate().map(|(i, c)| {
    ...
    match kriging_result {
        Ok(sr) => {
            v.check_pass("S6: Kriging predictions = target count", sr.values.len() == targets.len());
            v.check_pass("S6: all predictions finite", sr.values.iter().all(|v| v.is_finite()));
            v.check_pass("S6: all variances ≥ 0", sr.variances.iter().all(|&v| v >= 0.0));
    ...
    println!("╠═══════════════════════════════════════════════════════════════════╣");
    println!("║ {:30} │ {:>10.2} ms ║", "Stage 1: GPU Diversity (30×100)", s1_ms);
    println!("║ {:30} │ {:>10.2} ms ║", "Stage 2: Bootstrap CI (10k)", s2_ms);
    println!("║ {:30} │ {:>10.2} ms ║", "Stage 3: Jackknife Cross-Val", s3_ms);
    println!("║ {:30} │ {:>10.2} ms ║", "Stage 4: Regression Selection", s4_ms);
    println!("║ {:30} │ {:>10.2} ms ║", "Stage 5: PCoA Ordination", s5_ms);
    println!("║ {:30} │ {:>10.2} ms ║", "Stage 6: Kriging Interpolation", s6_ms);
    println!("╠═══════════════════════════════════════════════════════════════════╣");
    println!("║ {:30} │ {:>10.2} ms ║", "Sum of stages", roundtrip_ms);
    println!("║ {:30} │ {:>10.2} ms ║", "Pipeline total", total_ms);
    println!("║ {:30} │ {:>10.2} ms ║", "Overhead (scheduling)", total_ms - roundtrip_ms);
    println!("╚═══════════════════════════════════════════════════════════════════╝");
    println!();
