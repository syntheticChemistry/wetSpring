...
fn validate_hotspring_precision(
    v: &mut Validator,
    ...
    v.check("erf(1.0) — barracuda::special (hotSpring origin)", erf_val, 0.842_700_792_949_715, tolerances::GPU_LOG_POLYFILL);

    let lng_val = barracuda::special::ln_gamma(5.0).expect("ln_gamma");
    v.check("ln_gamma(5.0) — barracuda::special", lng_val, 3.178_053_830_347_95, tolerances::PYTHON_PARITY);

    let gamma_p = barracuda::special::regularized_gamma_p(1.0, 1.0).expect("gamma_p");
    v.check("regularized_gamma_p(1,1)", gamma_p, 1.0 - (-1.0_f64).exp(), tolerances::GPU_LOG_POLYFILL);

    let trapz_result = barracuda::numerical::trapz(
        &[0.0, 1.0, 4.0, 9.0],
        &[0.0, 1.0, 2.0, 3.0],
    ).expect("trapz");
    v.check("trapz(x²) [0,3]", trapz_result, 9.5, tolerances::EXACT);

    let (_, gemm_ms) = bench("GemmCached compile (f64 shader via hotSpring polyfills)", || {
        GemmCached::new(Arc::clone(device), barracuda::device::TensorContext::new(Arc::clone(device)).into())
    });
    v.check_pass("GEMM pipeline compiles", gemm_ms < 30_000.0);
...
fn validate_wetspring_bio_absorbed(
    v: &mut Validator,
    device: &Arc<WgpuDevice>,
    gpu: &GpuF64,
) {
    ...
    println!("  Evolution: wetSpring CPU → wetSpring WGSL → ToadStool absorbs → all springs benefit");
    ...
    v.check("Shannon CPU↔GPU parity", gpu_shannon, cpu_shannon, tolerances::GPU_VS_CPU_F64);

    let (_, ode_ms) = bench("Bistable ODE GPU (128 batches, trait-generated WGSL)", || {
        ...
        gpu_ode.integrate_params(&params, &initial, 500, 0.01).expect("integrate")
    });
    ...
    let data: Vec<f64> = (0..80).map(|i| (((i * 17 + 3) % 50) as f64) / 50.0 + 0.01).collect();
    let (nmf_result, _) = bench("NMF (10×8, k=3) — barracuda::linalg::nmf (wetSpring origin)", || {
        nmf::nmf(&data, 10, 8, &nmf_config)
    });
    let nmf_ok = nmf_result.as_ref().map(|r| r.w.iter().all(|&x| x >= 0.0)).unwrap_or(false);
...
fn validate_neuralspring_crossspring(
    v: &mut Validator,
    gpu: &GpuF64,
) {
    ...
    let vectors: Vec<Vec<f64>> = (0..n).map(|_| (0..dim).map(|_| rng.next_f64()).collect()).collect();
    ...
    v.check_pass("CPU pairwise distances computed", cpu_pairwise.len() == n * (n - 1) / 2);

    let (cpu_hamming, _) = bench("CPU Hamming (binary 64-bit)", || {
        let binary: Vec<u64> = (0..n).map(|i| (i as u64).wrapping_mul(0x5DEECE66D)).collect();
        let mut dists = Vec::with_capacity(n * (n - 1) / 2);
        for i in 0..n {
            for j in (i + 1)..n {
    ...
    v.check_pass("Hamming distances (neuralSpring PairwiseHammingGpu origin)", cpu_hamming.len() == n * (n - 1) / 2);

    let counts: Vec<f64> = (0..50).map(|_| rng.next_f64() * 100.0 + 1.0).collect();
    let (gpu_simpson, _) = bench("Simpson GPU (neuralSpring fitness landscape use case)", || {
        diversity_gpu::simpson_gpu(gpu, &counts).expect("GPU Simpson")
    });
    let cpu_simpson = diversity::simpson(&counts);
    v.check("Simpson CPU↔GPU (cross-spring diversity)", gpu_simpson, cpu_simpson, tolerances::GPU_VS_CPU_F64);
...
fn validate_track3_gpu_path(
    ...
    println!("  Evolution: CPU-only → ToadStool S58 (NMF) → S60 (TransE, SpMM, TopK) → S62 (PeakDetect)");
    ...
    let (gpu_dot, _) = bench("FMR dot (cosine component)", || fmr.dot(&a, &b).expect("dot"));
    let cpu_dot: f64 = a.iter().zip(b.iter()).map(|(x, y)| x * y).sum();
    v.check("FMR dot vs CPU", gpu_dot, cpu_dot, tolerances::GPU_VS_CPU_TRANSCENDENTAL);
    ...
    let scorer = TranseScoreF64 {
        entities: &entity_emb, relations: &relation_emb,
        n_entities: 100, n_relations: 4, dim: 16,
        heads: &heads, rels: &rels, tails: &tails,
    };
    ...
    let signal: Vec<f64> = (0..512).map(|i| {
        let x = i as f64 * 0.05;
        x.sin() + if (i > 100 && i < 120) || (i > 300 && i < 320) { 5.0 } else { 0.0 }
    }).collect();
    let params = signal::PeakParams { distance: 20, min_height: Some(3.0), ..Default::default() };
    ...
    match gpu_peaks_result {
        Ok(gpu_peaks) => {
            println!("  CPU: {} peaks, GPU: {} peaks", cpu_peaks.len(), gpu_peaks.len());
