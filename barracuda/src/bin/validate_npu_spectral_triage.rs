...
fn generate_spectrum(seed: u64, n_peaks: usize) -> Vec<(f64, f64)> {
    ...
    for _ in 0..n_peaks {
        let mz = MZ_MIN + lcg(&mut rng) * (MZ_MAX - MZ_MIN);
        let raw = lcg(&mut rng);
...
fn quantize_to_int8(v: &[f64]) -> Vec<i8> {
    let max_abs = v.iter().cloned().fold(0.0_f64, f64::max);
...
fn cosine_spectrum(a: &[(f64, f64)], b: &[(f64, f64)]) -> f64 {
    let (mz_a, int_a): (Vec<_>, Vec<_>) = a.iter().cloned().unzip();
    let (mz_b, int_b): (Vec<_>, Vec<_>) = b.iter().cloned().unzip();
...
fn main() {
    ...
    v.check_pass("each query has known match", true_match_idx.len() == N_QUERIES);
    ...
    let hist_nonzero = lib_hists.iter().all(|h: &Vec<f64>| h.iter().sum::<f64>() > 0.0)
        && query_hists.iter().all(|h: &Vec<f64>| h.iter().sum::<f64>() > 0.0);
    ...
    println!("  Pass rate (candidates / total): {:.4}", pass_rate);
    ...
    let recall = recall_count as f64 / N_QUERIES as f64;
    println!(
        "  Recall: {:.3} ({}/{} queries have true match in candidates)",
    ...
    let t_gpu_sample = t_gpu_start.elapsed();
    let gpu_scale = (N_QUERIES as f64 / sample_queries as f64)
        * (LIB_SIZE as f64 / sample_lib as f64).max(1.0);
    let t_gpu_extrapolated_s = t_gpu_sample.as_secs_f64() * gpu_scale;
    let speedup = t_gpu_extrapolated_s / t_npu.as_secs_f64().max(1e-9);
    println!("  NPU triage + GPU scoring: {:?}", t_npu);
    println!(
        "  GPU-only (extrap from {}q×{}lib): {:.1}s",
        sample_queries, sample_lib, t_gpu_extrapolated_s
    );
    ...
    println!("  NPU triage energy per query: {:.6} J", energy_per_query_j);
