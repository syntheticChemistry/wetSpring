...
pub fn accession_dir(db: &str, accession: &str) -> Result<PathBuf, String> {
    let base = cache_file(&format!("ncbi/{db}/{accession}"));
    std::fs::create_dir_all(&base)
        .map_err(|e| format!("cannot create accession dir {}: {e}", base.display()))?;
    Ok(base)
}
...
/// hashes do not match.
pub fn verify_integrity(dir: &Path, filename: &str) -> Result<(), String> {
    let file_path = dir.join(filename);
    let sidecar = dir.join(format!("{filename}.sha256"));
    ...
    let expected_hash = expected
        .split_whitespace()
        .next()
        .ok_or_else(|| "empty sidecar file".to_string())?;
...
mod tests {
    ...
    fn verify_integrity_missing_file() {
        let dir = tempfile::tempdir().unwrap();
        std::fs::write(
            dir.path().join("test.fasta.sha256"),
            "abc123  test.fasta\n",
        )
        .unwrap();
