...
pub fn store(socket: &Path, key: &str, value: &str) -> crate::error::Result<()> {
    let fam = active_family_id();
    let request = format!(
        r#"{{"jsonrpc":"2.0","method":"storage.store","params":{{"key":"{}","value":"{}","family_id":"{fam}"}},"id":1}}"#,
        escape_json(key),
        escape_json(value),
    );
...
pub fn retrieve(socket: &Path, key: &str) -> crate::error::Result<String> {
    let fam = active_family_id();
    let request = format!(
        r#"{{"jsonrpc":"2.0","method":"storage.retrieve","params":{{"key":"{}","family_id":"{fam}"}},"id":2}}"#,
        escape_json(key),
    );
    let response = rpc_call(socket, &request)?;
    if response.contains("\"error\"") {
...
pub fn exists(socket: &Path, key: &str) -> crate::error::Result<bool> {
    let fam = active_family_id();
    let request = format!(
        r#"{{"jsonrpc":"2.0","method":"storage.exists","params":{{"key":"{}","family_id":"{fam}"}},"id":3}}"#,
        escape_json(key),
    );
    let response = rpc_call(socket, &request)?;
