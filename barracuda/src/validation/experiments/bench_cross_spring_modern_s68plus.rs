...
/// Run the `benchmark_cross_spring_modern_s68plus` experiment, recording checks into `v`.
...
pub fn run_as_scenario(result: &mut primalspring::validation::ValidationResult) {
    let mut v = crate::validation::Validator::silent("benchmark_cross_spring_modern_s68plus");
    run(&mut v);
