...
fn compute_r_at_w(l: usize, w: f64, n_real: usize) -> (f64, f64) {
    ...
    for seed_offset in 0..n_real {
        let seed = crate::cast::usize_u64(42 + seed_offset * 1000);
    ...
    let stderr = (variance / crate::cast::usize_f64(n_real)).sqrt();
    (mean, stderr)
}
...
pub fn run_as_scenario(result: &mut primalspring::validation::ValidationResult) {
    let mut v = crate::validation::Validator::silent("validate_dynamic_anderson");
    run(&mut v);
