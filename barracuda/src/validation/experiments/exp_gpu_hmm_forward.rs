...
/// Run the `validate_gpu_hmm_forward` experiment, recording checks into `v`.
    ...
    match result {
        Ok(Ok(gpu_result)) => {
            ...
            v.check(
                ...
                cpu.log_likelihood,
                gpu_ll,
                crate::tolerances::GPU_VS_CPU_F64,
            );
            v.check(
...
pub fn run_as_scenario(result: &mut primalspring::validation::ValidationResult) {
    let mut v = crate::validation::Validator::silent("validate_gpu_hmm_forward");
    run(&mut v);
