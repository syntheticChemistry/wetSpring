...
/// Run the `validate_barracuda_gpu_full` experiment, recording checks into `v`.
pub fn run(v: &mut crate::validation::Validator) {
    ...
    __rt.block_on(async {
        ...
        v.section("G01: Shannon Entropy (FMR)");
        {
            ...
            let gpu_val = diversity_gpu::shannon_gpu(&gpu, &abundances).or_exit("GPU validation");
        ...
        v.section("G02: Simpson Diversity (FMR)");
        {
            ...
            let cpu_us = t_cpu.elapsed().as_micros() as f64;
            let t_gpu = Instant::now();
            let gpu_val = diversity_gpu::simpson_gpu(&gpu, &abundances).or_exit("GPU validation");
            let gpu_us = t_gpu.elapsed().as_micros() as f64;
            v.check(
                "Simpson: CPU == GPU",
                gpu_val,
        ...
        v.section("G03: Bray-Curtis (BrayCurtisF64)");
        {
            ...
            let t_gpu = Instant::now();
            let gpu_bc =
                diversity_gpu::bray_curtis_condensed_gpu(&gpu, &samples).or_exit("GPU validation");
            let gpu_us = t_gpu.elapsed().as_micros() as f64;
        ...
        v.section("G04: Spectral Cosine (FMR)");
        {
            ...
            let t_gpu = Instant::now();
            let gpu_self = spectral_match_gpu::pairwise_cosine_gpu(&gpu, &self_spectra)
                .or_exit("GPU validation");
            let gpu_pw = spectral_match_gpu::pairwise_cosine_gpu(&gpu, &spectra_gpu)
                .or_exit("GPU validation");
            let gpu_us = t_gpu.elapsed().as_micros() as f64;
            v.check(
            ...
            let all_valid = gpu_pw.iter().all(|s| {
        ...
        v.section("G05: Variance (FMR)");
        {
            ...
            let t_gpu = Instant::now();
            let gpu_var = stats_gpu::variance_gpu(&gpu, &data).or_exit("GPU validation");
            let gpu_us = t_gpu.elapsed().as_micros() as f64;
            v.check(
                "Variance: CPU == GPU",
                gpu_var,
        ...
        v.section("G06: ANI (ToadStool bio)");
        {
            ...
            let gpu_r = gpu_ani.batch_ani(&pairs).or_exit("GPU validation");
            let gpu_us = t_gpu.elapsed().as_micros() as f64;
        ...
        v.section("G08: dN/dS (ToadStool bio)");
        {
            ...
            let gpu_r = gpu_mod.batch_dnds(&pairs).or_exit("GPU validation");
            let gpu_us = t_gpu.elapsed().as_micros() as f64;
        ...
        v.section("G09: Pangenome (ToadStool bio)");
        {
            ...
            let gpu_r = gpu_pan.classify(&flat, 5, 4).or_exit("GPU validation");
            let gpu_us = t_gpu.elapsed().as_micros() as f64;
        ...
        v.section("G10: Random Forest (ToadStool bio)");
        {
            ...
            .or_exit("GPU validation");
            let rf = RandomForest::from_trees(vec![t1, t2, t3], 3).or_exit("GPU validation");
...
pub fn run_as_scenario(result: &mut primalspring::validation::ValidationResult) {
    let mut v = crate::validation::Validator::silent("validate_barracuda_gpu_full");
    run(&mut v);
...
pub const SCENARIO: crate::validation::scenarios::registry::Scenario =
    crate::validation::scenarios::registry::Scenario {
        meta: crate::validation::scenarios::registry::ScenarioMeta {
            id: "barracuda_gpu_full",
            track: crate::validation::scenarios::registry::Track::Science,
            tier: crate::validation::scenarios::registry::Tier::Both,
            provenance_crate: "validate_barracuda_gpu_full",
            provenance_date: "2026-05-20",
            description: "Exp071: `BarraCuda` GPU — Full Math Portability Proof",
        },
