...
pub fn run(v: &mut crate::validation::Validator) {
    ...
    v.section("G04: Spectral Cosine (FMR)");
    {
        ...
        let t_gpu = Instant::now();
        let gpu_self =
            spectral_match_gpu::pairwise_cosine_gpu(&gpu, &self_spectra).or_exit("GPU validation");
        let gpu_pw =
            spectral_match_gpu::pairwise_cosine_gpu(&gpu, &spectra_gpu).or_exit("GPU validation");
        let gpu_us = t_gpu.elapsed().as_micros() as f64;
        v.check(
