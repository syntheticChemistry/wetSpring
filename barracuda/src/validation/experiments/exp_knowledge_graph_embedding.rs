...
impl KgEmbedding {
    fn new(n_entities: usize, n_relations: usize, seed: u64) -> Self {
        ...
        for i in 0..n_entities {
            ...
            let norm: f64 = entity_embeddings[start..start + EMBED_DIM]
                ...
                .sum::<f64>()
                .sqrt();
            if norm > crate::tolerances::EMBEDDING_NORM_FLOOR {
    ...
    fn train_step(
        ...
        for &(h, r, t) in triples {
            let neg_t = loop {
                let candidate = crate::cast::f64_usize(
                    rng.next_f64() * crate::cast::usize_f64(self.n_entities),
                ) % self.n_entities;
                if candidate != t {
                    break candidate;
                }
        ...
        total_loss / crate::cast::usize_f64(triples.len())
    }
}
        ...
        for i in 0..drug_cluster_size {
            ...
            let disease = 100
                ...
                + crate::cast::f64_usize(
                    rng.next_f64() * crate::cast::usize_f64(disease_cluster_size),
...
fn validate_link_prediction(
    ...
    for &(h, r, t) in eval_triples {
        ...
        mean_rank += crate::cast::usize_f64(rank);
    }

    let hits10_pct = f64::from(hits_at_10) / crate::cast::usize_f64(n_eval);
    mean_rank /= crate::cast::usize_f64(n_eval);
...
fn validate_gpu_transe(v: &mut Validator, kg: &KgEmbedding, triples: &[(usize, usize, usize)]) {
    ...
    let heads: Vec<u32> = triples
        .iter()
        .map(|t| crate::cast::usize_u32(t.0))
        .collect();
    let rels: Vec<u32> = triples
        .iter()
        .map(|t| crate::cast::usize_u32(t.1))
        .collect();
    let tails: Vec<u32> = triples
        .iter()
        .map(|t| crate::cast::usize_u32(t.2))
        .collect();
...
pub fn run(v: &mut crate::validation::Validator) {
    ...
    validate_link_prediction(v, &kg, &triples);
...
pub fn run_as_scenario(result: &mut primalspring::validation::ValidationResult) {
    let mut v = crate::validation::Validator::silent("validate_knowledge_graph_embedding");
    run(&mut v);
