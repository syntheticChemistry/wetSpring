...
fn validate_tree_inference(device: &Arc<WgpuDevice>, v: &mut Validator) {
    ...
    match ti.predict(&forest, &samples, n_samples) {
        Ok(predictions) => {
            ...
            let cpu_tree = crate::bio::decision_tree::DecisionTree::from_arrays(
                &[0, -1, -1],
                &[5.0, 0.0, 0.0],
                &[1, -1, -1],
                &[2, -1, -1],
...
fn validate_smith_waterman(device: &Arc<WgpuDevice>, v: &mut Validator) {
    ...
    match result {
        Ok(Ok(result)) => {
            ...
            let cpu_score = crate::bio::alignment::smith_waterman_score(
                b"ACGTACGT",
                b"ACTTACTT",
                &crate::bio::alignment::ScoringParams {
                    match_score: 2,
                    mismatch_penalty: -1,
...
pub fn run_as_scenario(result: &mut primalspring::validation::ValidationResult) {
    let mut v = crate::validation::Validator::silent("validate_toadstool_bio");
    run(&mut v);
