...
/// Run the `validate_pure_gpu_streaming_v3` experiment, recording checks into `v`.
...
fn validate_quality_filter(
    ...
    let tg = Instant::now();
    let (gpu_filtered, gpu_stats) = session
        .filter_reads(&reads, &params)
...
pub fn run_as_scenario(result: &mut primalspring::validation::ValidationResult) {
    let mut v = crate::validation::Validator::silent("validate_pure_gpu_streaming_v3");
    run(&mut v);
