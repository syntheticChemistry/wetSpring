...
/// Run the `validate_barracuda_cpu_v8` experiment, recording checks into `v`.
pub fn run(v: &mut crate::validation::Validator) {
    let t_total = Instant::now();

    validate_cooperation(v);
    validate_capacitor(v);
    validate_kmd(v);
    validate_gbm(v);
    validate_merge_pairs(v);
    validate_signal(v);
    validate_feature_table(v);
    validate_robinson_foulds(v);
    validate_derep(v);
    validate_chimera(v);
    validate_neighbor_joining(v);
    validate_reconciliation(v);
...
pub fn run_as_scenario(result: &mut primalspring::validation::ValidationResult) {
    let mut v = crate::validation::Validator::silent("validate_barracuda_cpu_v8");
    run(&mut v);
