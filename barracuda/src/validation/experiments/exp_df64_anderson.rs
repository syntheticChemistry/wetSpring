...
fn compute_r_stats(l: usize, w: f64, n_real: usize) -> (f64, f64) {
    ...
    for seed_offset in 0..n_real {
        let seed = crate::cast::usize_u64(42 + seed_offset * 1000 + l * 100);
    ...
    #[cfg(feature = "gpu")]
    {
        ...
        if w_c_values.len() >= 2 {
            let mean_wc = w_c_values.iter().map(|(_, wc)| wc).sum::<f64>()
                / crate::cast::usize_f64(w_c_values.len());
...
pub fn run_as_scenario(result: &mut primalspring::validation::ValidationResult) {
    let mut v = crate::validation::Validator::silent("validate_df64_anderson");
    run(&mut v);
