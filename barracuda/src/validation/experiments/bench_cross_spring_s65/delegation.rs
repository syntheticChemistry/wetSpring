...
fn validate_diversity_fusion(
    ...
    let (fusion_gpu, fusion_gpu_ms) =
        bench_print("DiversityFusionGpu (256 samples, 10 spp)", || {
            let fusion = crate::bio::diversity_fusion_gpu::DiversityFusionGpu::new(
                Arc::clone(device),
            )
            .or_exit("DiversityFusionGpu");
            fusion.compute(&abundances, n_samples, n_species).or_exit("compute")
        });
    ...
    v.check_pass("DiversityFusion: GPU sample count", fusion_gpu.len() == n_samples);
    ...
    for i in 0..n_samples {
        if (fusion_cpu[i].shannon - fusion_gpu[i].shannon).abs() > tolerances::GPU_LOG_POLYFILL
            ...
            || (fusion_cpu[i].evenness - fusion_gpu[i].evenness).abs() > tolerances::GPU_LOG_POLYFILL
        {
            fusion_parity_ok = false;
        }
    }
    v.check_pass("DiversityFusion: CPU↔GPU parity (256 samples)", fusion_parity_ok);
    v.check_pass("DiversityFusion: all Shannon > 0", fusion_gpu.iter().all(|r| r.shannon > 0.0));
    timings.push(BenchRow { label: "DiversityFusion GPU 256×", origin: "wetSpring→S63", ms: fusion_gpu_ms });
    timings.push(BenchRow { label: "DiversityFusion CPU 256×", origin: "wetSpring→S63", ms: fusion_cpu_ms });
}
...
fn validate_cpu_diversity(v: &mut Validator, timings: &mut Vec<BenchRow>) {
    ...
    v.check("Shannon delegation parity", sh_local, sh_upstream, tolerances::EXACT);
    ...
    v.check("Simpson delegation parity", si_local, si_upstream, tolerances::EXACT);
    ...
    v.check("Chao1 delegation parity", ch_local, barracuda::stats::chao1(&community), tolerances::EXACT);

    let (pe_local, _) = bench_print("diversity::pielou_evenness (→ barracuda::stats)", || {
        crate::bio::diversity::pielou_evenness(&community)
    });
    v.check("Pielou delegation parity", pe_local, barracuda::stats::pielou_evenness(&community), tolerances::EXACT);
    ...
    v.check("Bray-Curtis delegation parity", bc_local, barracuda::stats::bray_curtis(&samples_a, &samples_b), tolerances::EXACT);

    let multi_samples = vec![vec![10.0, 20.0, 30.0], vec![15.0, 10.0, 25.0], vec![0.0, 50.0, 0.0]];
    ...
    let bc_cond_parity = bc_cond_local.iter().zip(&bc_cond_upstream).all(|(a, b)| (a - b).abs() <= tolerances::EXACT);
    ...
    let rare_parity = rare_local.iter().zip(&rare_upstream).all(|(a, b)| (a - b).abs() <= tolerances::EXACT);
    v.check_pass("rarefaction_curve delegation parity", rare_parity);
    timings.push(BenchRow { label: "Shannon delegation", origin: "S64 cross-spring", ms: sh_ms });
    timings.push(BenchRow { label: "Simpson delegation", origin: "S64 cross-spring", ms: si_ms });
}
...
fn validate_cpu_math(v: &mut Validator, timings: &mut Vec<BenchRow>) {
    ...
    let vec_b: Vec<f64> = (0..1000).map(|i| f64::from(i).mul_add(-0.001, 1.0)).collect();

    let (dot_local, dot_ms) = bench_print("special::dot (→ barracuda::stats::dot)", || {
        crate::special::dot(&vec_a, &vec_b)
    });
    v.check("dot delegation parity", dot_local, barracuda::stats::dot(&vec_a, &vec_b), tolerances::EXACT);

    let (l2_local, l2_ms) = bench_print("special::l2_norm (→ barracuda::stats::l2_norm)", || {
        crate::special::l2_norm(&vec_a)
    });
    v.check("l2_norm delegation parity", l2_local, barracuda::stats::l2_norm(&vec_a), tolerances::EXACT);
    timings.push(BenchRow { label: "dot(1000) delegation", origin: "S64 cross-spring", ms: dot_ms });
    timings.push(BenchRow { label: "l2_norm(1000) delegation", origin: "S64 cross-spring", ms: l2_ms });
}
...
fn validate_special_functions(v: &mut Validator, timings: &mut Vec<BenchRow>) {
    ...
    let (erf_val, erf_ms) = bench_print("erf(1.0) — barracuda::special", || barracuda::special::erf(1.0));
    v.check("erf(1.0)", erf_val, 0.842_700_792_949_715, tolerances::ERF_PARITY);
    timings.push(BenchRow { label: "erf(1.0)", origin: "hotSpring→ToadStool", ms: erf_ms });

    let (lng_val, lng_ms) = bench_print("ln_gamma(5.0) — barracuda::special", || {
        barracuda::special::ln_gamma(5.0).or_exit("ln_gamma")
    });
    v.check("ln_gamma(5.0)", lng_val, 3.178_053_830_347_95, tolerances::PYTHON_PARITY);
    timings.push(BenchRow { label: "ln_gamma(5.0)", origin: "hotSpring→ToadStool", ms: lng_ms });

    let (ncdf_val, ncdf_ms) = bench_print("norm_cdf(1.96) — barracuda::stats", || barracuda::stats::norm_cdf(1.96));
    v.check("norm_cdf(1.96) ≈ 0.975", ncdf_val, 0.975, tolerances::NORM_CDF_PARITY);
    timings.push(BenchRow { label: "norm_cdf(1.96)", origin: "hotSpring→ToadStool", ms: ncdf_ms });
    ...
    v.check("∫x² dx [0,0.999]", trapz_val, 0.332_334, tolerances::ODE_METHOD_PARITY);
    timings.push(BenchRow { label: "trapz(1000)", origin: "ToadStool native", ms: trapz_ms });
    ...
    timings.push(BenchRow { label: "pearson(1000)", origin: "ToadStool native", ms: pear_ms });
}
