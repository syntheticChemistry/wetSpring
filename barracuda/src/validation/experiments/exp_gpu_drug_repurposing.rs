...
fn validate_gpu_peak_detection(
    ...
    let t_cpu = Instant::now();
    let cpu_peaks = crate::bio::signal::find_peaks(
        &signal,
        &crate::bio::signal::PeakParams {
            distance: 50,
            min_height: Some(5.0),
            min_prominence: None,
            min_width: None,
...
/// Run the `validate_gpu_drug_repurposing` experiment, recording checks into `v`.
pub fn run(v: &mut crate::validation::Validator) {
    ...
    validate_gemm_nmf_reconstruction(v, &gemm, &mut timings);
    validate_gpu_cosine_similarity(v, &fmr, &mut timings);
    validate_gpu_transe(v, &device, &mut timings);
    validate_gpu_peak_detection(v, &device, &mut timings);
    validate_sparse_gemm(v, &device, &mut timings);
    validate_topk_ranking(v, &mut timings);
...
pub fn run_as_scenario(result: &mut primalspring::validation::ValidationResult) {
    let mut v = crate::validation::Validator::silent("validate_gpu_drug_repurposing");
    run(&mut v);
