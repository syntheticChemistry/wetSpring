...
use crate::bio::{dada2, diversity, phred, unifrac};
use crate::tolerances;
use crate::validation::Validator;

/// Run the `validate_r_industry_parity` experiment, recording checks into `v`.
pub fn run(v: &mut crate::validation::Validator) {
    let t0 = Instant::now();

    validate_vegan_shannon_simpson(v);
    validate_vegan_bray_curtis(v);
    validate_vegan_rarefaction(v);
    validate_vegan_chao1(v);
    validate_vegan_pielou(v);
    validate_dada2_constants(v);
    validate_dada2_error_model(v);
    validate_dada2_phred(v);
    validate_dada2_consensus(v);
    validate_phyloseq_unifrac(v);
    validate_phyloseq_cophenetic(v);
...
pub fn run_as_scenario(result: &mut primalspring::validation::ValidationResult) {
    let mut v = crate::validation::Validator::silent("validate_r_industry_parity");
    run(&mut v);
...
pub const SCENARIO: crate::validation::scenarios::registry::Scenario = crate::validation::scenarios::registry::Scenario {
    meta: crate::validation::scenarios::registry::ScenarioMeta {
        id: "r_industry_parity",
        track: crate::validation::scenarios::registry::Track::Pipeline,
        tier: crate::validation::scenarios::registry::Tier::Rust,
        provenance_crate: "validate_r_industry_parity",
        provenance_date: "2026-05-20",
        description: "R Industry Parity Validation — vegan / DADA2 / phyloseq",
    },
