...
/// Run the `validate_pure_gpu_streaming_v12` experiment, recording checks into `v`.
pub fn run(v: &mut crate::validation::Validator) {
    ...
    v.check(
        "S3: Gompertz H(50) → P",
        *g_batch.last().or_exit("unexpected error"),
        350.0,
        1.0,
    );
    s3 += 1;
...
pub fn run_as_scenario(result: &mut primalspring::validation::ValidationResult) {
    let mut v = crate::validation::Validator::silent("validate_pure_gpu_streaming_v12");
    run(&mut v);
