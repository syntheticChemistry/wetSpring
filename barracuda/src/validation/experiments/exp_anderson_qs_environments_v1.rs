...
pub fn run(v: &mut crate::validation::Validator) {
    ...
    fn correlation(predicted: &[f64], known: &[f64]) -> f64 {
        let n = crate::cast::usize_f64(predicted.len());
    ...
    let mae = |predicted: &[f64], known: &[f64]| -> f64 {
        predicted
            ...
            .map(|(p, k)| (p - k).abs())
            .sum::<f64>()
            / crate::cast::usize_f64(predicted.len())
    };
...
pub fn run_as_scenario(result: &mut primalspring::validation::ValidationResult) {
    let mut v = crate::validation::Validator::silent("validate_anderson_qs_environments_v1");
    run(&mut v);
