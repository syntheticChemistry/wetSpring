...
//!
//! Provenance: metalForge extended validation (V11)
...
/// Run the `validate_metalforge_v11_extended` experiment, recording checks into `v`.
...
pub fn run_as_scenario(result: &mut primalspring::validation::ValidationResult) {
    let mut v = crate::validation::Validator::silent("validate_metalforge_v11_extended");
    run(&mut v);
