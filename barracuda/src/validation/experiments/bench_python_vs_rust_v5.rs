...
#[expect(
    dead_code,
    reason = "benchmark: struct fields populated for future reporting"
)]
struct ParityBench {
    domain: &'static str,
...
/// Run the `benchmark_python_vs_rust_v5` experiment, recording checks into `v`.
...
pub fn run_as_scenario(result: &mut primalspring::validation::ValidationResult) {
    let mut v = crate::validation::Validator::silent("benchmark_python_vs_rust_v5");
    run(&mut v);
