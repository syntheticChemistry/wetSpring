...
/// Run the `benchmark_progression_cpu_gpu_stream` experiment, recording checks into `v`.
...
pub fn run_as_scenario(result: &mut primalspring::validation::ValidationResult) {
    let mut v = crate::validation::Validator::silent("benchmark_progression_cpu_gpu_stream");
    run(&mut v);
