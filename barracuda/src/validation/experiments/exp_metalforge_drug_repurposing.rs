...
/// Run the `validate_metalforge_drug_repurposing` experiment, recording checks into `v`.
pub fn run(v: &mut crate::validation::Validator) {
    ...
    validate_nmf_reconstruction_parity(v, &gemm, &mut timings);
    validate_cosine_scoring_parity(v, &fmr, &mut timings);
    validate_transe_parity(v, &gpu, &mut timings);
    validate_drug_ranking_parity(v, &gemm);
...
pub fn run_as_scenario(result: &mut primalspring::validation::ValidationResult) {
    let mut v = crate::validation::Validator::silent("validate_metalforge_drug_repurposing");
    run(&mut v);
