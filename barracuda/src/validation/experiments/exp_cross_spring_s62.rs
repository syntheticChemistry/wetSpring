...
/// Run the `validate_cross_spring_s62` experiment, recording checks into `v`.
pub fn run(v: &mut crate::validation::Validator) {
    ...
    validate_hotspring_precision(v, &device);
    validate_wetspring_bio_absorbed(v, &device, &gpu);
    validate_neuralspring_crossspring(v, &gpu);
    validate_track3_gpu_path(v, &device, &gpu);
    print_evolution_narrative(v);
...
pub fn run_as_scenario(result: &mut primalspring::validation::ValidationResult) {
    let mut v = crate::validation::Validator::silent("validate_cross_spring_s62");
    run(&mut v);
