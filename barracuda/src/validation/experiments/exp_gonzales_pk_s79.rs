...
/// Run the `validate_gonzales_pk_s79` experiment, recording checks into `v`.
pub fn run(v: &mut crate::validation::Validator) {
    ...
    for (i, (&dose, &dur)) in doses_mg_kg.iter().zip(durations_days.iter()).enumerate() {
        ...
        v.check_pass(
            ...
            (peak - dose).abs() / dose < crate::tolerances::PK_PEAK_DOSE_RELATIVE,
        );
        v.check_pass(
            &format!("Dose {dose:.3}: final << peak"),
            final_conc < peak * crate::tolerances::PK_ELIMINATION_FLOOR,
        );
...
pub fn run_as_scenario(result: &mut primalspring::validation::ValidationResult) {
    let mut v = crate::validation::Validator::silent("validate_gonzales_pk_s79");
    run(&mut v);
