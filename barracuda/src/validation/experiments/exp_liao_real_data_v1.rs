...
/// Run the `validate_liao_real_data_v1` experiment, recording checks into `v`.
pub fn run(v: &mut crate::validation::Validator) {
    ...
    v.check_pass(
        ...
        communities.iter().all(|c| {
            ...
            (s - 1.0).abs() < crate::tolerances::ABUNDANCE_SUM_TO_ONE
        }),
    );
...
pub fn run_as_scenario(result: &mut primalspring::validation::ValidationResult) {
    let mut v = crate::validation::Validator::silent("validate_liao_real_data_v1");
    run(&mut v);
