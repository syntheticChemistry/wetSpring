...
#[expect(
    ...
    reason = "validation harness: sequential feature validation parse→tracks→EIC→features→asari cross-reference"
)]
...
pub fn run_as_scenario(result: &mut primalspring::validation::ValidationResult) {
    let mut v = crate::validation::Validator::silent("validate_features");
    run(&mut v);
