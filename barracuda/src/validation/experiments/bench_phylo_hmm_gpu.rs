...
/// Run the `benchmark_phylo_hmm_gpu` experiment, recording checks into `v`.
...
fn bench_bootstrap(device: &Arc<barracuda::device::WgpuDevice>, v: &mut Validator) {
    ...
    let start = Instant::now();
    let mut rng = crate::bio::gillespie::Lcg64::new(42);
...
pub fn run_as_scenario(result: &mut primalspring::validation::ValidationResult) {
    let mut v = crate::validation::Validator::silent("benchmark_phylo_hmm_gpu");
    run(&mut v);
