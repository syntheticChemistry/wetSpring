...
pub fn run(v: &mut crate::validation::Validator) {
    ...
    let (bc_local, bc_ms) = bench_print("diversity::bray_curtis (→ barracuda::stats)", || {
    ...
    let (gemm_res, first_ms) = bench_print("GEMM first dispatch (256×128 × 128×256)", || {
...
pub fn run_as_scenario(result: &mut primalspring::validation::ValidationResult) {
    let mut v = crate::validation::Validator::silent("benchmark_cross_spring_s68");
    run(&mut v);
