...
use crate::io::nanopore::{NanoporeRead, SyntheticSignalGenerator};
...
pub fn run(v: &mut crate::validation::Validator) {
    ...
    validate_feature_extraction(v, &features);
    validate_quantization(v, &features);
    validate_classification(v, &batch);
...
pub fn run_as_scenario(result: &mut primalspring::validation::ValidationResult) {
    let mut v = crate::validation::Validator::silent("validate_nanopore_int8_quantization");
    run(&mut v);
