...
//! 3. Diversity metrics: `crate::bio::diversity` vs. textbook formulas
...
/// Run the `validate_gonzales_cpu_parity` experiment, recording checks into `v`.
...
pub fn run_as_scenario(result: &mut primalspring::validation::ValidationResult) {
    let mut v = crate::validation::Validator::silent("validate_gonzales_cpu_parity");
    run(&mut v);
