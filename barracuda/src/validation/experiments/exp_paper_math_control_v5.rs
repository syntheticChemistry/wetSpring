...
/// Run the `validate_paper_math_control_v5` experiment, recording checks into `v`.
pub fn run(v: &mut crate::validation::Validator) {
    ...
    let rf_tree = crate::bio::unifrac::PhyloTree::from_newick("((A,B),(C,D));");
...
pub fn run_as_scenario(result: &mut primalspring::validation::ValidationResult) {
    let mut v = crate::validation::Validator::silent("validate_paper_math_control_v5");
    run(&mut v);
