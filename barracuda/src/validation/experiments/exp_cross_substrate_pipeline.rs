...
use crate::bio::{diversity, diversity_gpu, stats_gpu};
...
fn make_communities() -> Vec<Vec<f64>> {
    (0..N_SAMPLES)
        .map(|s| {
            (0..N_FEATURES)
                .map(|f| {
                    let base = (crate::cast::usize_f64(s * N_FEATURES + f + 1)).sqrt();
                    let gradient =
                        (crate::cast::usize_f64(s) / crate::cast::usize_f64(N_SAMPLES)) * 2.0;
...
/// Run the `validate_cross_substrate_pipeline` experiment, recording checks into `v`.
pub fn run(v: &mut crate::validation::Validator) {
    ...
    __rt.block_on(async {
        ...
        let upload_us = crate::cast::u128_f64(t_upload.elapsed().as_micros());
        ...
        for community in &communities {
            ...
            let observed = diversity_gpu::observed_features_gpu(&gpu, community)
                .or_exit("observed features GPU");
        ...
        let gpu_compute_us = crate::cast::u128_f64(t_gpu_compute.elapsed().as_micros());

        let t_bray = Instant::now();
        let gpu_bray =
            diversity_gpu::bray_curtis_condensed_gpu(&gpu, &communities).or_exit("Bray-Curtis GPU");
        let bray_us = crate::cast::u128_f64(t_bray.elapsed().as_micros());

        let t_readback = Instant::now();
        let _readback_copy = gpu_bray.clone();
        let readback_us = crate::cast::u128_f64(t_readback.elapsed().as_micros());
        ...
        let classify_us = crate::cast::u128_f64(t_classify.elapsed().as_micros());
        ...
        let mean_shannon: f64 = sample_results.iter().map(|r| r.shannon).sum::<f64>()
            / crate::cast::usize_f64(N_SAMPLES);
        ...
        let n_low = sample_results
            ...
            .count();
        let agg_us = crate::cast::u128_f64(t_agg.elapsed().as_micros());
        ...
        v.check(
            ...
            crate::cast::usize_f64(n_high + n_mod + n_low),
            crate::cast::usize_f64(N_SAMPLES),
            tolerances::EXACT,
        );
        v.check(
        ...
        let cpu_mean_shannon: f64 =
            cpu_results.iter().map(|(s, _, _)| s).sum::<f64>() / crate::cast::usize_f64(N_SAMPLES);
        v.check(
...
pub fn run_as_scenario(result: &mut primalspring::validation::ValidationResult) {
    let mut v = crate::validation::Validator::silent("validate_cross_substrate_pipeline");
    run(&mut v);
...
pub const SCENARIO: crate::validation::scenarios::registry::Scenario =
    crate::validation::scenarios::registry::Scenario {
        meta: crate::validation::scenarios::registry::ScenarioMeta {
            id: "cross_substrate_pipeline",
            track: crate::validation::scenarios::registry::Track::Pipeline,
            tier: crate::validation::scenarios::registry::Tier::Both,
            provenance_crate: "validate_cross_substrate_pipeline",
            provenance_date: "2026-05-20",
            description: "Exp076: `metalForge` Cross-Substrate Pipeline",
        },
