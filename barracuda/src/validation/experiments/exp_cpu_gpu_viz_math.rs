...
/// Run the `validate_cpu_gpu_viz_math` experiment, recording checks into `v`.
pub fn run(v: &mut crate::validation::Validator) {
    ...
    validate_alpha_diversity(v, &gpu, &counts);
...
pub fn run_as_scenario(result: &mut primalspring::validation::ValidationResult) {
    let mut v = crate::validation::Validator::silent("validate_cpu_gpu_viz_math");
    run(&mut v);
