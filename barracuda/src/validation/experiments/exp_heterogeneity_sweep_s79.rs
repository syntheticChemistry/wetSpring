...
/// Run the `validate_heterogeneity_sweep_s79` experiment, recording checks into `v`.
...
pub fn run_as_scenario(result: &mut primalspring::validation::ValidationResult) {
    let mut v = crate::validation::Validator::silent("validate_heterogeneity_sweep_s79");
    run(&mut v);
