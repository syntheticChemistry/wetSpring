...
/// Run the `validate_barracuda_cpu_v9` experiment, recording checks into `v`.
pub fn run(v: &mut crate::validation::Validator) {
    let t_total = Instant::now();

    validate_pathway_scoring(v);
    validate_matrix_pharmacophenomics(v);
    validate_nmf_drug_repurposing(v);
    validate_repodb_nmf(v);
    validate_kg_embedding(v);
...
pub fn run_as_scenario(result: &mut primalspring::validation::ValidationResult) {
    let mut v = crate::validation::Validator::silent("validate_barracuda_cpu_v9");
    run(&mut v);
