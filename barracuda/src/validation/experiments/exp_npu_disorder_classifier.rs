...
fn generate_diversity_profile(seed: u64, regime: usize) -> Vec<f64> {
    ...
    vec![
        ...
        ((regime as f64).mul_add(100.0, 50.0) + noise(seed, 2) * 50.0) / 400.0,
...
pub fn run(v: &mut crate::validation::Validator) {
    ...
    for regime in 0..N_REGIMES {
        ...
        let indices: Vec<usize> = test_true
            .iter()
            .enumerate()
            .filter(|&(_, t)| *t == regime)
            .map(|(i, _)| i)
    ...
    let avg_w_by_regime: Vec<f64> = (0..N_REGIMES)
        .map(|r| {
            let indices: Vec<usize> = test_true
                .iter()
                .enumerate()
        ...
        .collect();
    println!(
        "  Avg W by regime: propagating={:.3}, intermediate={:.3}, localized={:.3}",
...
pub fn run_as_scenario(result: &mut primalspring::validation::ValidationResult) {
    let mut v = crate::validation::Validator::silent("validate_npu_disorder_classifier");
    run(&mut v);
