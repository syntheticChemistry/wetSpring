...
fn generate_emp_scale_synthetic(n_per_biome: usize) -> Vec<OtuSample> {
    ...
    for (biome, o2, mean_h, o2_regime) in &biomes {
        for i in 0..n_per_biome {
            let seed = crate::cast::usize_u64(biome.len())
...
fn compute_diversity(sample: &OtuSample) -> DiversityResult {
    ...
    let pielou = if richness > 1 {
        shannon / crate::cast::usize_f64(richness).ln()
    } else {
        0.0
    };
...
/// Run the `validate_emp_anderson_v1` experiment, recording checks into `v`.
pub fn run(v: &mut crate::validation::Validator) {
    ...
    let mean_anaerobic = if anaerobic.is_empty() {
        ...
        anaerobic.iter().sum::<f64>() / crate::cast::usize_f64(anaerobic.len())
    };
    let mean_aerobic = if aerobic.is_empty() {
        0.0
    } else {
        aerobic.iter().sum::<f64>() / crate::cast::usize_f64(aerobic.len())
    };
    ...
    #[cfg(feature = "json")]
    {
        ...
        use crate::visualization::{DataChannel, EcologyScenario, ScenarioNode};
        ...
        atlas_node.data_channels.push(DataChannel::Gauge {
            ...
            value: crate::cast::usize_f64(biome_summary.len()),
            min: 0.0,
            max: 30.0,
            unit: "biomes".into(),
...
pub fn run_as_scenario(result: &mut primalspring::validation::ValidationResult) {
    let mut v = crate::validation::Validator::silent("validate_emp_anderson_v1");
    run(&mut v);
