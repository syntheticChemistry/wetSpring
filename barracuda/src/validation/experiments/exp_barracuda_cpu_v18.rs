...
/// Run the `validate_barracuda_cpu_v18` experiment, recording checks into `v`.
pub fn run(v: &mut crate::validation::Validator) {
    ...
    let diversity_data = [
        2.15, 2.31, 1.98, 2.42, 2.05, 2.28, 2.11, 2.37, 2.19, 2.25, 2.33, 2.08, 2.22, 2.40, 2.03,
        2.29, 2.14, 2.36, 2.20, 2.27,
    ];
    ...
    println!(
        "  Bootstrap: {:.4} [{:.4}, {:.4}] SE={:.4}",
        ci.estimate, ci.lower, ci.upper, ci.std_error
    );
    ...
    let sample_mean: f64 = barracuda::stats::mean(&abundances);
    v.check(
        "RAWR estimate ≈ sample mean",
        rawr.estimate,
        sample_mean,
        2.0,
    );
    ...
    println!(
        "  RAWR: {:.4} [{:.4}, {:.4}] SE={:.4}",
        rawr.estimate, rawr.lower, rawr.upper, rawr.std_error
    );
    ...
    println!(
        "  Bootstrap SE = {bs_se:.6}, Jackknife SE = {jk_se:.6}, ratio = {:.2}",
        bs_se / jk_se
    );
    ...
    if let Some(ref fe) = fit_exp {
        ...
        println!(
            "  Exponential: params={:?}, R²={:.6}, RMSE={:.6}",
            fe.params, fe.r_squared, fe.rmse
        );
    ...
    if let Some(ref fq) = fit_quad {
        ...
        println!(
            "  Quadratic: params={:?}, R²={:.6}",
            fq.params, fq.r_squared
        );
    }
    ...
    if let Some(ref fl) = fit_log {
        ...
        println!(
            "  Logarithmic: params={:?}, R²={:.6}",
            fl.params, fl.r_squared
        );
    }
    ...
    let genomes: Vec<f64> = (1..=50).map(f64::from).collect();
    ...
    for &p in &rare_abundances {
        ...
        v.check_pass(
            &format!("p={p}: depth achieves ≥ 0.95 power"),
            actual >= 0.95,
        );
    }

    let obs_data: Vec<f64> = rare_abundances
        .iter()
...
pub fn run_as_scenario(result: &mut primalspring::validation::ValidationResult) {
    let mut v = crate::validation::Validator::silent("validate_barracuda_cpu_v18");
    run(&mut v);
