...
/// Run the `validate_kbs_lter_anderson_v1` experiment, recording checks into `v`.
pub fn run(v: &mut crate::validation::Validator) {
    ...
    let notill_mean_p: f64 = {
        let pts: Vec<&TimePoint> = all_points
            ...
            .collect();
        pts.iter().map(|p| p.p_qs).sum::<f64>() / crate::cast::usize_f64(pts.len())
    };
    let conv_mean_p: f64 = {
        let pts: Vec<&TimePoint> = all_points
            .iter()
...
pub fn run_as_scenario(result: &mut primalspring::validation::ValidationResult) {
    let mut v = crate::validation::Validator::silent("validate_kbs_lter_anderson_v1");
    run(&mut v);
