...
/// Run the `validate_metalforge_v12_extended` experiment, recording checks into `v`.
pub fn run(v: &mut crate::validation::Validator) {
    ...
    let ipc_qs = dispatch::dispatch("science.qs_model", &json!({})).or_exit("IPC QS");
...
pub fn run_as_scenario(result: &mut primalspring::validation::ValidationResult) {
    let mut v = crate::validation::Validator::silent("validate_metalforge_v12_extended");
    run(&mut v);
