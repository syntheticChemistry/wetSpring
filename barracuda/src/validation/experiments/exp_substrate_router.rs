...
fn route_shannon(router: &SubstrateRouter, gpu: Option<&GpuF64>, counts: &[f64]) -> RouterResult {
    ...
    RouterResult {
        value,
        substrate: target,
        us: crate::cast::u128_f64(t.elapsed().as_micros()),
    }
}
...
fn route_bray_curtis(
    ...
    (
        value,
        target,
        crate::cast::u128_f64(t.elapsed().as_micros()),
    )
}

/// Run the `validate_substrate_router` experiment, recording checks into `v`.
...
pub fn run_as_scenario(result: &mut primalspring::validation::ValidationResult) {
    let mut v = crate::validation::Validator::silent("validate_substrate_router");
    run(&mut v);
