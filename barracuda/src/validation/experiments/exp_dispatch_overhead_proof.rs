...
/// Run the `validate_dispatch_overhead_proof` experiment, recording checks into `v`.
...
pub fn run_as_scenario(result: &mut primalspring::validation::ValidationResult) {
    let mut v = crate::validation::Validator::silent("validate_dispatch_overhead_proof");
    run(&mut v);
