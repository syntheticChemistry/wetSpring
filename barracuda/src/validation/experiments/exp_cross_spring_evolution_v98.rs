...
/// Run the `validate_cross_spring_evolution_v98` experiment, recording checks into `v`.
pub fn run(v: &mut crate::validation::Validator) {
    ...
    // ═══ §7: wetSpring NMF + Drug Repurposing ══════════════════════════
    {
        ...
        let data: Vec<f64> = (0..m * n)
            .map(|i| (crate::cast::usize_f64(i % 7) + 1.0) / 7.0)
            .collect();
    ...
    #[cfg(feature = "gpu")]
    {
        ...
        let h_cpu = crate::bio::diversity::shannon(&community);
        v.check(
            "GPU Shannon ≈ CPU",
            h_gpu,
            h_cpu,
        ...
        let s_cpu = crate::bio::diversity::simpson(&community);
        v.check(
            "GPU Simpson ≈ CPU",
            s_gpu,
            s_cpu,
        ...
        match bc_gpu {
            Ok(ref bc_vec) => {
                let bc_cpu = crate::bio::diversity::bray_curtis(&community, &b);
                v.check_pass(
                    "GPU BC ≈ CPU",
...
pub fn run_as_scenario(result: &mut primalspring::validation::ValidationResult) {
    let mut v = crate::validation::Validator::silent("validate_cross_spring_evolution_v98");
    run(&mut v);
