...
/// Run the `validate_barracuda_cpu_v24` experiment, recording checks into `v`.
pub fn run(v: &mut crate::validation::Validator) {
    ...
    let dt = crate::bio::decision_tree::DecisionTree::from_arrays(
        &[0, -1, -1],
        &[3.0, 0.0, 0.0],
        &[1, -1, -1],
        &[2, -1, -1],
...
pub fn run_as_scenario(result: &mut primalspring::validation::ValidationResult) {
    let mut v = crate::validation::Validator::silent("validate_barracuda_cpu_v24");
    run(&mut v);
