...
pub fn run(v: &mut crate::validation::Validator) {
    ...
    validate_anderson_1d(v);
    validate_almost_mathieu(v);
    validate_lanczos_vs_sturm(v);
    validate_anderson_2d(v);
    validate_anderson_3d_transition(v);
    validate_qs_disorder_analogy(v);
...
pub fn run_as_scenario(result: &mut primalspring::validation::ValidationResult) {
    let mut v = crate::validation::Validator::silent("validate_spectral_cross_spring");
    run(&mut v);
