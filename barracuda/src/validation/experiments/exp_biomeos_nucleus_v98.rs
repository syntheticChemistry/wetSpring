...
pub fn run(v: &mut crate::validation::Validator) {
    ...
    if has_runtime {
        let biomeos_dir = PathBuf::from(runtime_dir.as_ref().or_exit("unexpected error"))
            .join(crate::ipc::primal_names::BIOMEOS);
...
pub fn run_as_scenario(result: &mut primalspring::validation::ValidationResult) {
    let mut v = crate::validation::Validator::silent("validate_biomeos_nucleus_v98");
    run(&mut v);
