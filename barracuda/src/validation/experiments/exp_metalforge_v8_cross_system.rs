...
/// Run the `validate_metalforge_v8_cross_system` experiment, recording checks into `v`.
...
pub fn run_as_scenario(result: &mut primalspring::validation::ValidationResult) {
    let mut v = crate::validation::Validator::silent("validate_metalforge_v8_cross_system");
    run(&mut v);
