...
use crate::bio::alignment::{ScoringParams, score_batch, smith_waterman_score};
use crate::bio::felsenstein::{FlatTree, TreeNode, encode_dna, log_likelihood};
use crate::bio::hmm::{HmmModel, forward, forward_batch, viterbi, viterbi_batch};
use crate::bio::neighbor_joining::{distance_matrix, neighbor_joining};
use crate::bio::reconciliation::{DtlCosts, FlatRecTree, reconcile_dtl};
...
/// Run the `validate_barracuda_cpu_v2` experiment, recording checks into `v`.
pub fn run(v: &mut crate::validation::Validator) {
    validate_flat_felsenstein(v);
    validate_batch_hmm(v);
    validate_batch_sw(v);
    validate_nj_and_dtl(v);
    validate_cross_module(v);
}

/// Bridge into [`primalspring::validation::ValidationResult`] for UniBin dispatch.
pub fn run_as_scenario(result: &mut primalspring::validation::ValidationResult) {
    let mut v = crate::validation::Validator::silent("validate_barracuda_cpu_v2");
    run(&mut v);
