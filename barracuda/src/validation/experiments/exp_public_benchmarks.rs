...
pub fn run(v: &mut crate::validation::Validator) {
    ...
    temporal_analysis(v, &all_results);

    // ── Cross-dataset benchmark ─────────────────────────────────────────
    cross_dataset_benchmark(v, &all_results);
...
fn process_sample(
    ...
    let records = match crate::io::fastq::FastqIter::open(&fastq_path)
...
pub fn run_as_scenario(result: &mut primalspring::validation::ValidationResult) {
    let mut v = crate::validation::Validator::silent("validate_public_benchmarks");
    run(&mut v);
