...
/// Run the `validate_barracuda_cpu_v6` experiment, recording checks into `v`.
pub fn run(v: &mut crate::validation::Validator) {
    let t_total = Instant::now();

    validate_qs_biofilm(v);
    validate_bistable(v);
    validate_multi_signal(v);
    validate_phage_defense(v);
...
pub fn run_as_scenario(result: &mut primalspring::validation::ValidationResult) {
    let mut v = crate::validation::Validator::silent("validate_barracuda_cpu_v6");
    run(&mut v);
