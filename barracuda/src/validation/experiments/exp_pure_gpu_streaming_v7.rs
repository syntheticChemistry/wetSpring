...
//! | Date | 2026-03-03 |
//! | Command | `cargo run --release --bin validate_pure_gpu_streaming_v7` |
//!
//! Provenance: Pure GPU streaming pipeline (V7)
...
/// Run the `validate_pure_gpu_streaming_v7` experiment, recording checks into `v`.
pub fn run(v: &mut crate::validation::Validator) {
    ...
    let gpu_shannon = diversity_gpu::shannon_gpu(&gpu, &combined).or_exit("unexpected error");
    ...
    let gpu_simpson = diversity_gpu::simpson_gpu(&gpu, &combined).or_exit("unexpected error");
    ...
    for run in 0..3 {
        let s = diversity_gpu::shannon_gpu(&gpu, &combined).or_exit("unexpected error");
        shannons.push(s);
...
pub fn run_as_scenario(result: &mut primalspring::validation::ValidationResult) {
    let mut v = crate::validation::Validator::silent("validate_pure_gpu_streaming_v7");
    run(&mut v);
