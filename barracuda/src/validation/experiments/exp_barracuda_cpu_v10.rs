...
/// Run the `validate_barracuda_cpu_v10` experiment, recording checks into `v`.
pub fn run(v: &mut crate::validation::Validator) {
    let t_total = Instant::now();

    validate_diversity_pipeline(v);
    validate_bray_curtis(v);
    validate_dynamic_anderson(v);
    validate_npu_quantization(v);
    validate_fasta_to_diversity(v);
...
pub fn run_as_scenario(result: &mut primalspring::validation::ValidationResult) {
    let mut v = crate::validation::Validator::silent("validate_barracuda_cpu_v10");
    run(&mut v);
