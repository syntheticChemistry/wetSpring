...
/// Run the `validate_gpu_v59_science` experiment, recording checks into `v`.
pub fn run(v: &mut crate::validation::Validator) {
    let t_total = Instant::now();

    validate_gpu_anderson(v);
    validate_diversity_anderson_pipeline(v);
    validate_wc_determination(v);
    validate_cold_seep_spectral(v);
...
pub fn run_as_scenario(result: &mut primalspring::validation::ValidationResult) {
    let mut v = crate::validation::Validator::silent("validate_gpu_v59_science");
    run(&mut v);
