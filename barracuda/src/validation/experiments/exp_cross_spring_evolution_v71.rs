...
/// Run the `validate_cross_spring_evolution_v71` experiment, recording checks into `v`.
pub fn run(v: &mut crate::validation::Validator) {
    ...
    let pearson = barracuda::stats::pearson_correlation(&obs, &sim).or_exit("pearson");
    v.check(
        "Pearson(linear) ≈ 1.0",
        pearson,
        1.0,
...
pub fn run_as_scenario(result: &mut primalspring::validation::ValidationResult) {
    let mut v = crate::validation::Validator::silent("validate_cross_spring_evolution_v71");
    run(&mut v);
