...
/// Run the `benchmark_three_tier` experiment, recording checks into `v`.
...
fn load_python_baseline() -> Option<serde_json::Value> {
    let path = std::env::var("WETSPRING_PYTHON_BASELINE").map_or_else(
        |_| validation::discover_bench_dir().join("python_baseline_latest.json"),
...
pub fn run_as_scenario(result: &mut primalspring::validation::ValidationResult) {
    let mut v = crate::validation::Validator::silent("benchmark_three_tier");
    run(&mut v);
...
pub const SCENARIO: crate::validation::scenarios::registry::Scenario = crate::validation::scenarios::registry::Scenario {
    meta: crate::validation::scenarios::registry::ScenarioMeta {
        ...
        provenance_crate: "benchmark_three_tier",
        provenance_date: "2026-05-20",
        description: "Exp069: Python → Rust CPU → GPU Three-Tier Benchmark",
    },
