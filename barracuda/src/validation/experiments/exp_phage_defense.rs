...
/// Run the `validate_phage_defense` experiment, recording checks into `v`.
...
pub fn run_as_scenario(result: &mut primalspring::validation::ValidationResult) {
    let mut v = crate::validation::Validator::silent("validate_phage_defense");
    run(&mut v);
