...
/// Run the `validate_primal_pipeline_v1` experiment, recording checks into `v`.
pub fn run(v: &mut crate::validation::Validator) {
    ...
    let neural_api_present = crate::ipc::provenance::neural_api_socket().is_some();
    println!(
        "  Neural API: {}",
...
pub fn run_as_scenario(result: &mut primalspring::validation::ValidationResult) {
    let mut v = crate::validation::Validator::silent("validate_primal_pipeline_v1");
    run(&mut v);
