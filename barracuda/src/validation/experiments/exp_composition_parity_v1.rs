...
//! | Field | Value |
//! |-------|-------|
//! | Baseline | Local Rust math via crate::bio / Python scripts |
...
pub fn run(v: &mut crate::validation::Validator) {
    ...
    for (name, source) in graph_sources {
        let result = crate::facade::graph_validate::validate_graph(source);
        v.check_pass(
...
pub fn run_as_scenario(result: &mut primalspring::validation::ValidationResult) {
    let mut v = crate::validation::Validator::silent("validate_composition_parity_v1");
    run(&mut v);
