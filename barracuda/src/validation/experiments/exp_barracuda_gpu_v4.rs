...
pub fn run(v: &mut crate::validation::Validator) {
    ...
    validate_gpu_diversity(v);
    validate_gpu_bray_curtis(v);
    validate_gpu_qs_model(v);
    validate_gpu_anderson(v);
    validate_gpu_full_pipeline(v);
...
pub fn run_as_scenario(result: &mut primalspring::validation::ValidationResult) {
    let mut v = crate::validation::Validator::silent("validate_barracuda_gpu_v4");
    run(&mut v);
