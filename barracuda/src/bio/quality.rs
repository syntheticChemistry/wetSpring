...
pub fn find_adapter_3prime(
    ...
    // Check all possible alignment positions where adapter overlaps read end
    // Start from the leftmost position where we have at least min_overlap
    ...
    // Also check for adapter fully contained within the read
    if adp_len <= seq_len {
