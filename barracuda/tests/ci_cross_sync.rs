...
//! primalSpring canonical registry (`config/capability_registry.toml`, 458 methods).
//!
//! Per primalSpring Wave 20 directive: "CI cross-sync — validate your local
//! capability methods against primalSpring canonical 458. Zero drift is the target."
//!
//! Seven checks:
...
//! 6. Canonical primalSpring registry accessible and non-trivial (458+)
...
/// Cross-check: canonical primalSpring registry file exists and contains 458+
...
fn canonical_registry_accessible_and_nontrivial() {
    ...
    assert!(
        method_count >= 458,
        "canonical registry has only {method_count} method entries — expected 458+"
    );
}
