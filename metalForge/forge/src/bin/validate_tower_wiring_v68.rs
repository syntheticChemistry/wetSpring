...
fn main() {
    ...
    check("Parse empty Songbird result", parsed.is_empty(), &mut pass, &mut fail, &mut total);
    ...
    check("Parse GPU gate from Songbird", parsed.len() == 1, &mut pass, &mut fail, &mut total);
    if let Some(s) = parsed.first() {
        check("Strandgate is GPU", s.kind == SubstrateKind::Gpu, &mut pass, &mut fail, &mut total);
        check("Strandgate has f64", s.has(&Capability::F64Compute), &mut pass, &mut fail, &mut total);
        check("Strandgate has shader", s.has(&Capability::ShaderDispatch), &mut pass, &mut fail, &mut total);
        check("Strandgate origin is Mesh", matches!(&s.origin, SubstrateOrigin::Mesh { gate_name } if gate_name == "strandgate"), &mut pass, &mut fail, &mut total);
    }
    ...
    check("Parse 3 gates", parsed.len() == 3, &mut pass, &mut fail, &mut total);
    check("Gate 3 is NPU", parsed.get(2).is_some_and(|s| s.kind == SubstrateKind::Npu), &mut pass, &mut fail, &mut total);
    ...
    check("Error response returns empty", parsed.is_empty(), &mut pass, &mut fail, &mut total);
    println!();
    ...
    if let Some(ref vibrio_path) = vibrio_res.path {
        ...
        let total_bytes: u64 = assemblies.iter()
        ...
        check("Vibrio path available", false, &mut pass, &mut fail, &mut total);
    }
    ...
    if let Some(ref campy_path) = campy_res.path {
        ...
        check("Campylobacterota path available", false, &mut pass, &mut fail, &mut total);
    }
    println!();
    ...
    check(
        ...
        local_subs.iter().all(|s| s.origin == SubstrateOrigin::Local),
        &mut pass, &mut fail, &mut total,
    );

    let songbird_available = inventory::discover_songbird_socket().is_some();
    println!("  Songbird socket: {}", if songbird_available { "FOUND" } else { "not running (standalone mode)" });

    let local_count = tower_subs.iter().filter(|s| s.origin == SubstrateOrigin::Local).count();
    ...
    let vibrio_workload = Workload::new(
        "vibrio_landscape",
    ...
    if let Some(d) = &decision {
        println!("  Vibrio routed to: {} ({:?})", d.substrate.identity.name, d.reason);
    }
    ...
    if spectral_decision.is_some() {
        check("PFAS spectral match routes to GPU", true, &mut pass, &mut fail, &mut total);
    } else {
        check("PFAS spectral match falls back to CPU (no GPU)", true, &mut pass, &mut fail, &mut total);
    }
    println!();
    ...
    if let Some(ref pfas_path) = pfas_res.path {
        ...
        let has_joseph = files.iter().any(|f| {
            f.file_name().to_string_lossy().contains("Joseph")
        });
        check(
        ...
        check("PFAS path available", false, &mut pass, &mut fail, &mut total);
    }
    println!();
...
fn split_json_objects(content: &str) -> Vec<&str> {
    ...
    for (i, ch) in content.char_indices() {
        match ch {
            ...
            '}' => { depth -= 1; if depth == 0 { if let Some(s) = start { objects.push(&content[s..=i]); } start = None; } }
            _ => {}
        }
    }
    objects
...
fn parse_service_entry(json: &str) -> Option<Substrate> {
    ...
    for cap in &caps_raw {
        match cap.as_str() {
            "f64" => { capabilities.push(Capability::F64Compute); has_f64 = true; }
            "gpu" | "shader" => { kind = SubstrateKind::Gpu; capabilities.push(Capability::ShaderDispatch); }
            "npu" | "quant" => { kind = SubstrateKind::Npu; capabilities.push(Capability::QuantizedInference { bits: 8 }); }
            "reduce" => { capabilities.push(Capability::ScalarReduce); }
            _ => {}
        }
    }
    ...
    Some(Substrate {
        ...
        properties: Properties { has_f64, ..Properties::default() },
        capabilities,
...
fn extract_array(json: &str, key: &str) -> Vec<String> {
    let pat = format!("\"{key}\"");
    let Some(s) = json.find(&pat) else { return Vec::new(); };
    let after = &json[s + pat.len()..];
    let Some(arr_s) = after.find('[') else { return Vec::new(); };
    let Some(arr_e) = after[arr_s..].find(']') else { return Vec::new(); };
