...
**Current release — V199:** Wave 107 cross-subnet mesh. Transport-aware capability discovery for cross-gate resolution (UDS + TCP + mesh relay). southGate `192.168.4.29/22` — cross-subnet from eastGate/ironGate, mesh ready. **Clippy zero warnings** (all feature combinations). 2,100 tests (0 failures). 345 scenarios, 51 niche, 59 consumed, 46 dispatch, 22 domains. 0 PG gaps open.
