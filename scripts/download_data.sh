...
#
# Usage:
#   ./scripts/download_data.sh [--all | --validation | --algae | --phage | --pfas | --t2]
#
# Track 1 (Life Science): --validation, --algae, --phage
# Track 2 (PFAS/blueFish): --pfas, --t2
# Both: --all
#
...
# ══════════════════════════════════════════════════════════════════
#  DATASET 6: asari Demo LC-MS Data (Track 2)
# ══════════════════════════════════════════════════════════════════
#
# Source: shuzhao-li-lab/data on GitHub — demo mzML files for asari
# Purpose: Validate asari installation and establish LC-MS processing baseline
# Reference: Nature Communications 14, 4113 (2023)

if [[ "$MODE" == "--all" || "$MODE" == "--pfas" || "$MODE" == "--t2" ]]; then
    echo "── Dataset 6: asari Demo LC-MS Data ──────────────────────"
    ASARI_DIR="$DATA_DIR/asari-demo-data"
    if [ -d "$ASARI_DIR/.git" ]; then
        echo "  [SKIP] asari demo data — already cloned"
    else
        echo "  [DOWNLOAD] Cloning shuzhao-li-lab/data..."
        git clone --depth 1 https://github.com/shuzhao-li-lab/data.git "$ASARI_DIR" 2>/dev/null || \
            echo "  [WARN] Clone failed — check network. Can retry later."
    fi
    echo
fi

# ══════════════════════════════════════════════════════════════════
#  DATASET 8: EPA CompTox PFAS Structure List (Track 2)
# ══════════════════════════════════════════════════════════════════
#
# Source: EPA CompTox Dashboard — PFAS Master List
# Purpose: Reference database for PFAS suspect screening
# Contains: ~14,000 PFAS structures with identifiers, properties

if [[ "$MODE" == "--all" || "$MODE" == "--pfas" || "$MODE" == "--t2" ]]; then
    echo "── Dataset 8: EPA CompTox PFAS List ──────────────────────"
    mkdir -p "$DATA_DIR/epa-comptox"
    echo "  [NOTE] EPA CompTox PFAS list requires manual download from:"
    echo "         https://comptox.epa.gov/dashboard/chemical-lists/PFASMASTER"
    echo "         Export as CSV → save to data/epa-comptox/PFASMASTER.csv"
    echo "         (~14,000 PFAS structures with CAS, DTXSID, SMILES)"
    echo
fi

# ══════════════════════════════════════════════════════════════════
#  DATASET 9: MassBank PFAS Reference Spectra (Track 2)
# ══════════════════════════════════════════════════════════════════
#
# Source: MassBank.eu — open mass spectral database
# Purpose: Reference MS2 spectra for PFAS compound matching
# Format: MassBank record format + msp

if [[ "$MODE" == "--all" || "$MODE" == "--pfas" || "$MODE" == "--t2" ]]; then
    echo "── Dataset 9: MassBank PFAS Reference Spectra ────────────"
    MASSBANK_DIR="$DATA_DIR/massbank"
    mkdir -p "$MASSBANK_DIR"
    download_url \
        "https://github.com/MassBank/MassBank-data/releases/latest/download/MassBank_NIST.msp" \
        "$MASSBANK_DIR/MassBank_NIST.msp" \
        "MassBank NIST MSP (all spectra, includes PFAS)"
    echo
fi

# ══════════════════════════════════════════════════════════════════
#  DATASET 10: Michigan DEQ PFAS Water Monitoring (Track 2)
# ══════════════════════════════════════════════════════════════════
#
# Source: Michigan Department of Environment, Great Lakes, and Energy
# Purpose: Training data for ML models predicting PFAS in drinking water
# Reference: Nejadhashemi et al., Water Research (2023)
...
echo "#" >> "$MANIFEST"
find "$DATA_DIR" -type f \( -name "*.fastq.gz" -o -name "*.fna.gz" -o -name "*.qza" -o -name "*.tar.gz" -o -name "*.mzML" -o -name "*.msp" -o -name "*.csv" \) \
